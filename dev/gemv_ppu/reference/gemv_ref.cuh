/*
 * q4k_gemv.h - Q4_K quantized GEMV for Hopper-class GPUs
 *
 * Computes  out[m, n] = act[m, k] * W[n, k]^T  where W is stored in the GGUF
 * Q4_K block format and the activation is fp16. Optimized for the GEMV shape
 * (small m, typically 1), i.e. decode-phase matrix-vector products.
 *
 * ---------------------------------------------------------------------------
 * Storage format (fixed by GGUF, see block_q4_K below)
 * ---------------------------------------------------------------------------
 * One super-block covers QK_K = 256 weights in 144 bytes:
 *
 *   offset  0.. 1   half     d       super-block scale
 *   offset  2.. 3   half     dmin    super-block min-scale
 *   offset  4..15   uint8    scales[12]   8 x (6-bit sc, 6-bit m), packed
 *   offset 16..143  uint8    qs[128]      256 x 4-bit weights
 *
 * The 256 weights form 8 groups of 32; group j has its own (sc[j], m[j]) pair
 * unpacked from scales[12] by get_scale_min_k4_w().
 *
 * Nibble interleave inside qs: byte b holds two weights that are 32 elements
 * apart, not adjacent:
 *
 *   low  nibble of qs[b] -> element (b % 32) + (b / 32) * 64
 *   high nibble of qs[b] -> element (b % 32) + (b / 32) * 64 + 32
 *
 * Dequantization, algebraically rearranged so the 4-bit value can be turned
 * into fp16 by bit tricks (see lop3_convert_to_h2):
 *
 *   w = d * sc * q - dmin * m
 *     = (q - 8) * scale + zero        with  scale = d * sc
 *                                          zero  = 8 * scale - dmin * m
 *
 * ---------------------------------------------------------------------------
 * Work decomposition
 * ---------------------------------------------------------------------------
 * Three compile-time parameters:
 *
 *   CTA_N    columns handled by one warp
 *   WARPS_N  warp groups along the column axis (each group owns CTA_N columns)
 *   WARPS_K  warps that split the k axis inside a group (partial sums are
 *            combined through shared memory in the epilogue)
 *
 *   CTA size          = WARPS_N * WARPS_K * 32 threads   (<= 1024)
 *   columns per CTA   = CTA_N * WARPS_N
 *   grid              = (m, n / (CTA_N * WARPS_N))
 *
 * Inside a warp the k axis is fixed:
 *
 *   32 lanes = 4 super-block groups x THREADS_PER_SB(8) lanes
 *   one warp covers BLOCKS_PER_WARP(4) super-blocks = ELEMS_PER_WARP(1024)
 *   elements of k per loop iteration ("chunk")
 *
 * Lane -> data mapping, with sb = lane_id / 8 and t = lane_id % 8:
 *
 *   super-block index  = chunk * 4 + sb
 *   qs bytes owned     = [t * 16, t * 16 + 16)   -> 4 uint32 words, 32 weights
 *   low  nibbles cover k_lo + [0, 16)  with k_lo = (t / 2) * 64 + (t % 2) * 16
 *   high nibbles cover k_lo + 32 + [0, 16)
 *   scale groups used  = j_lo = (t / 2) * 2  and  j_hi = j_lo + 1
 *
 * So every lane needs exactly two (sc, m) pairs and two 16-element runs of the
 * activation, 32 bytes apart. That is what the two float4 pairs below load.
 *
 * ---------------------------------------------------------------------------
 * Memory strategy
 * ---------------------------------------------------------------------------
 * The activation row is read once per CTA into dynamic shared memory
 * (k * sizeof(half) bytes) by a fully coalesced cooperative copy, followed by
 * the only __syncthreads() in the kernel. Shared memory is read-only from then
 * on, so the steady-state loop needs no further barriers.
 *
 * Without this, every warp of the CTA would issue the same activation loads to
 * global memory: with WARPS_K = 1 the address of a lane's activation does not
 * depend on warp_id, so all WARPS_N * WARPS_K warps would request identical
 * addresses every iteration.
 *
 * Per loop iteration the remaining global traffic is, per column, one 16-byte
 * load of the super-block header (d, dmin, scales) and one 16-byte load of the
 * lane's quant bytes. Both are issued before any arithmetic so the requests
 * overlap.
 *
 * The 16-byte header load is performed by all 8 lanes of a super-block group
 * from the same address; the hardware coalesces that into a single
 * transaction, which is cheaper than having lane 0 load and broadcasting with
 * __shfl_sync.
 *
 * Alignment invariant: sizeof(block_q4_K) == 144 == 16 * 9, so with a
 * 16-byte-aligned array base every super-block starts 16-byte aligned and both
 * uint4 loads are legal. See the static_assert below.
 *
 * ---------------------------------------------------------------------------
 * Numeric path
 * ---------------------------------------------------------------------------
 * scale and zero are computed in fp16. zero must be evaluated as a fused
 * multiply-add from (8*d, sc) rather than from the already-rounded scale: the
 * (q - 8) form relies on the anchor term 8*scale cancelling the bias, and
 * rounding scale first would scale that error by 8 and destroy the
 * cancellation.
 *
 * Dequantized weights are never materialized. Each 32-bit quant word is turned
 * into four half2 values holding (q - 8) and consumed immediately by two
 * chained __hfma2 operations (dequantize, then multiply-accumulate).
 *
 * Products accumulate in fp16 across four independent half2 chains (chain
 * length 4, one per quant word) and are folded into an fp32 accumulator once
 * per super-block, which bounds fp16 accumulation error while keeping the inner
 * loop in half2 arithmetic.
 *
 * ---------------------------------------------------------------------------
 * Constraints
 * ---------------------------------------------------------------------------
 *   k % ELEMS_PER_WARP == 0        (k must be a multiple of 1024)
 *   n % (CTA_N * WARPS_N) == 0
 *   WARPS_N * WARPS_K * 32 <= 1024
 *   activation and weight base pointers 16-byte aligned (cudaMalloc satisfies)
 *
 * WARPS_K does not have to divide k / ELEMS_PER_WARP; the strided chunk loop
 * stays correct, warps inside a group then differ by at most one chunk of work.
 */
...
q_h2x4 lop3_convert_to_h2(uint32_t i4s_raw)
{
    ...
    asm volatile("sub.f16x2 %0, %1, %2;\n" : "=r"(h[0]) : "r"(h[0]), "n"(FP16_TOP_MAGIC));
    asm volatile("sub.f16x2 %0, %1, %2;\n" : "=r"(h[2]) : "r"(h[2]), "n"(FP16_TOP_MAGIC));
    ...
    asm volatile("fma.rn.f16x2 %0, %1, %2, %3;\n" : "=r"(h[1]) : "r"(h[1]), "n"(ONE_SIXTEENTH), "n"(NEG_72));
    asm volatile("fma.rn.f16x2 %0, %1, %2, %3;\n" : "=r"(h[3]) : "r"(h[3]), "n"(ONE_SIXTEENTH), "n"(NEG_72));
...
// ----------------------------------------------------------------------------
// u6_pair_to_half2: two 6-bit integers -> half2, no conversion instruction.
// Same 0x6400 bit-insert trick as the weight decode: OR a small integer into
// the mantissa of fp16(1024) to get fp16(1024 + v) exactly (valid for
// v <= 1023), then subtract 1024. Packs a pair so the two scale groups convert
// together (sc_lo/sc_hi in one call, m_lo/m_hi in another).
// ----------------------------------------------------------------------------
__device__ __forceinline__
half2 u6_pair_to_half2(uint32_t lo, uint32_t hi)   // lo, hi in [0, 63]
{
    union { uint32_t u; half2 h2; } v, bias;
    v.u    = 0x64006400u | (lo & 0xFF) | ((hi & 0xFF) << 16);
    bias.u = 0x64006400u;                          // half2{1024, 1024}
...
#pragma unroll
        for (int ii = 0; ii < CTA_N; ++ii)
        {
            ...
            // sc/m -> fp16 via the same 0x6400 bit-insert trick as the weight
            // decode (no conversion instruction). Pack the two scale groups:
            //   sc2 = (sc_lo, sc_hi),  m2 = (m_lo, m_hi)
            half2 const sc2 = u6_pair_to_half2(sc_lo, sc_hi);
            half2 const m2  = u6_pair_to_half2(m_lo,  m_hi);

            // scale = d*sc, zero = 8*d*sc - dmin*m, both groups (lo, hi) at once
            half2 const scale2 = __hmul2(__half2half2(d), sc2);
            half2 const zero2  = __hfma2(__half2half2(d8), sc2,
                                         __hmul2(__half2half2(dminn), m2));

            // broadcast each group's scalar to a half2 for the paired dot product
            half2 const scale_lo2 = __half2half2(__low2half(scale2));
            half2 const scale_hi2 = __half2half2(__high2half(scale2));
            half2 const zero_lo2  = __half2half2(__low2half(zero2));
            half2 const zero_hi2  = __half2half2(__high2half(zero2));
