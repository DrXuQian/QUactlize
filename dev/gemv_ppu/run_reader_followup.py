#!/usr/bin/env python3
"""Screen remaining cold Q4 shapes, confirm three candidates, and retain ACU evidence."""
import argparse
import csv
import ctypes as C
import json
...
from dev.gemv_ppu.run_config_sweep import SweepBench, execute, PREFIX, FAIL_PREFIX, shortlist
...
def recipe(variant, n, k, key):
    if variant == 'reader':
        return spec.lookup(n, k, key).recipe
    if variant not in ANCHORS or key != 'control':
        raise ValueError('unknown followup arm')
    if variant == 'baseline':
        c = spec.baseline(n, k)
        return c.recipe[1:] if c.family == 'affine' else (1, 16, 1)
    return control_recipe(variant, n, k)


def arithmetic(variant, n, k, key):
    if variant == 'reader':
        return spec.lookup(n, k, key).arithmetic
    return spec.baseline(n, k).arithmetic if variant == 'baseline' else 'PER_WEIGHT_FP16'


def control_kind(candidate):
    return 'IMMUTABLE_CONFIG_BINARY' if candidate.family == 'affine' else 'UNCHANGED_SMALL_BODY_SAME_GEOMETRY'


class FollowupBench(SweepBench):
    def __init__(self, a):
        super().__init__(SimpleNamespace(**(vars(a) | dict(candidate=a.config_bundle))))
        self.followup_libraries, self.followup_launch = {}, {}
        self.reader_allowed = {c.recipe:c for c in spec.inventory(self.n, self.k)}
        for family in sorted({c.family for c in self.reader_allowed.values()}):
            lib = C.CDLL(str(a.candidate/spec.payload(self.n, self.k, family)), mode=C.RTLD_LOCAL)
            probe = lib.q4_ppu_probe
            probe.argtypes = [C.POINTER(C.c_int)]*3+[C.c_char_p]
            probe.restype = C.c_int
            l2, sm, warp, name = C.c_int(), C.c_int(), C.c_int(), C.create_string_buffer(256)
            checked(probe(C.byref(l2), C.byref(sm), C.byref(warp), name), 'followup image marker')
            if (sm.value, warp.value, name.value.decode()) != (self.device['sm'], self.device['warp'], self.device['name']):
                raise ValueError('followup image device differs')
            fn = getattr(lib, 'q4_followup_small_run' if family == 'small' else f'q4_followup_run_{self.n}_{self.k}')
            fn.argtypes = [C.c_int]*(3 if family == 'small' else 4)+[C.c_void_p]*5
            fn.restype = C.c_int
            self.followup_libraries[family], self.followup_launch[family] = lib, fn
        self.matched, self.historical_match = None, None

    def invoke(self, cfg, index=0, force_aiu=None):
        if force_aiu is not None or tuple(cfg) not in self.reader_allowed:
            raise ValueError('followup config not compiled')
        c = self.reader_allowed[tuple(cfg)]
        low, units = self.weight_pointers[index % self.copies]
        return self.followup_launch[c.family](*cfg, self.a, low, units, self.output, self.r.stream)

    def direct(self, cfg, force_aiu=None):
        c = self.reader_allowed[tuple(cfg)]
        low, units = self.weight_pointers[0]
        self.historical_match = None
        self.r.fill(self.output_base, 0xff, self.output_bytes+32)
        if c.family == 'affine':
            rc = self.config_launch(*cfg[1:], self.a, low, units, self.output, self.r.stream)
        else:
            rc = self.followup_launch['small'](0, *cfg[1:], self.a, low, units, self.output, self.r.stream)
        checked(rc, 'unchanged same-geometry control')
        err = self.error()
        if not math.isfinite(err) or err >= .005:
            raise ValueError('same-geometry control failed independent GGUF oracle')
        old = self.sdk.download(self.output, self.output_bytes)
        if c.family == 'small' and tuple(cfg[1:]) == (8, 16):
            self.r.fill(self.output_base, 0xff, self.output_bytes+32)
            checked(self.current_launch(self.n, self.k, self.a, low, units, self.output, self.r.stream),
                    'historical small binary control')
            old_err = self.error()
            if not math.isfinite(old_err) or old_err >= .005:
                raise ValueError('historical small control failed independent GGUF oracle')
            self.historical_match = exact_bits(old, self.sdk.download(self.output, self.output_bytes),
                                               'unchanged small body vs immutable historical binary')
            err = max(err, old_err)
        new, error = super().direct(cfg, force_aiu=force_aiu)
        self.matched = exact_bits(old, new, 'followup vs same-geometry FP32 output')
        return new, max(err, error)


def child(a):
    spec.verify(a.candidate, a.reuse, a.config_bundle, a.previous, a.controls, a.bundle, sources=False)
    keys = json.loads(a.keys)
    if not keys or len(set(keys)) != len(keys):
        raise ValueError('empty/duplicate child keys')
    bench = None
    try:
        if a.variant == 'reader':
            bench = FollowupBench(a)
        elif a.variant == 'baseline':
            # Fixture dimensions, not filename spelling, choose the historical family.
            import numpy as np
            with np.load(a.fixture, allow_pickle=False) as f:
                small = spec.baseline(int(f['n']), int(f['k'])).family == 'small'
            bench = (PortBench(SimpleNamespace(**(vars(a) | dict(candidate=a.controls, variant='kpack'))))
                     if small else SweepBench(SimpleNamespace(**(vars(a) | dict(candidate=a.config_bundle)))))
        else:
            bench = PortBench(SimpleNamespace(**(vars(a) | dict(candidate=a.controls))))
        n, k = bench.n, bench.k
        if (n, k) not in spec.SHAPES:
            raise ValueError('shape outside followup')
        for key in keys:
            try:
                ...
                if a.variant == 'reader':
                    ...
                    row.update(family=c.family, geometry=c.geometry(n, k), code_dequant='LOP3_HALF2',
                        matched_fp32_sha256=bench.matched, historical_small_sha256=bench.historical_match,
                        same_geometry_control=control_kind(c), access_models=[spec.access(c, n, k, dict(A=aa, B=bb, metadata=mm))
                ...
                traceback.print_exc()
                return 1  # Preserve preceding cells. Remaining keys get a fresh child/context.
        return 0
    finally:
        if bench:
            bench.close()
...
def parse_row(raw, variant, n, k, key, phase, samples, profile=False):
    ...
    if (row.get('variant') != variant or row.get('config_key') != key or row.get('phase') != phase or
        ...
        raise ValueError('followup identity/precision/cache differs')
    ...
    if variant == 'reader':
        ...
        if (row.get('family') != c.family or row.get('geometry') != c.geometry(n,k) or
            row.get('same_geometry_control') != control_kind(c) or row.get('code_dequant') != 'LOP3_HALF2' or
            not hash_ok(row.get('matched_fp32_sha256'))):
            raise ValueError('reader mapping or matched raw-bit proof differs')
        historical = c.family == 'small' and c.recipe[1:] == (8,16)
        if (historical and not hash_ok(row.get('historical_small_sha256'))) or (not historical and row.get('historical_small_sha256') is not None):
            raise ValueError('historical small binary proof differs')
        models, seen = row.get('access_models', []), set()
        if not models:
            raise ValueError('missing observed load patterns')
        for model in models:
            bases = model['first_warp']['base_mod128'] if c.family == 'affine' else model['base_mod128']
...
def run(a):
    m = spec.verify(a.candidate, a.reuse, a.config_bundle, a.previous, a.controls, a.bundle)
    device = probe_device(a)  # Exiting child owns the probe context, never the orchestrator.
    names = set(m['source_hashes']) | {'dev/gemv_ppu/run_reader_followup.py', 'dev/gemv_ppu/run_config_sweep.py',
    ...
    authority = dict(schema='quactlize.q4-reader-followup-run.v1', candidate=digest(a.candidate/'manifest.json'),
    ...
    if diff:
        print('Q4_FOLLOWUP_SDK_DIFFERENCE recorded='+','.join(diff)+' real_marker_and_numeric_required=1',flush=True)
    ...
    def batch(variant,n,k,keys,phase,profile=False):
        ...
        while missing:
            ...
            cmd = [sys.executable,'-u',str(Path(__file__).resolve()),'--child','--sdk',str(a.sdk),
                '--candidate',str(a.candidate),'--reuse',str(a.reuse),'--config-bundle',str(a.config_bundle),
            ...
            print(f'Q4_FOLLOWUP_PROGRESS shape=1x{n}x{k} arm={variant} phase={phase} cells={len(missing)} elapsed_s={time.monotonic()-started:.1f}',flush=True)
            ...
            if missing:
                print(f'Q4_FOLLOWUP_RESTART remaining={len(missing)} failed={len(failed)} valid={len(cache)}',flush=True)
    ...
    for n,k in spec.SHAPES:
        ...
        print('Q4_FOLLOWUP_RESULT '+json.dumps(case['comparison']),flush=True)
        if not a.skip_acu:
            ...
            for variant,key in [*(('reader',key) for key in profile_keys),*((v,'control') for v in ANCHORS)]:
                ...
                save()
    spec.verify(a.candidate,a.reuse,a.config_bundle,a.previous,a.controls,a.bundle)
    ...
    print(f'Q4_FOLLOWUP_COMPLETE status={result["status"]} within_5pct={result["within_5pct"]}/4 prior_closed=2/2 results={a.output}',flush=True)
...
def main():
    ...
    for name,folder in [('candidate','q4-reader-followup-v1'),('reuse','q4-reader-reuse-v1'),('config-bundle','q4-config-sweep-v1'),
    ...
    if a.l2_bytes < 0:
        p.error('invalid L2 size')
    for name in ('sdk','candidate','reuse','config_bundle','previous','controls','bundle'):
        setattr(a,name,getattr(a,name).resolve(strict=True))
    if a.child:
        if a.fixture is None or a.variant is None or a.keys is None:
            p.error('child requires fixture/variant/keys')
