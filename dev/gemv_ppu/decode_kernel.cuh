...
// Only expose Columns for the existing frozen reader. Dequant, accumulation
// and CTA reduction remain the production q4_s1_readers.cuh bodies.
namespace q4_decode_sweep {
...
int launch(qkg_call_v1 const& c) {
    ...
    if(c.input_type==QKG_F16)
        kernel<0,R,V,W,P,Columns,N,K><<<c.rows*(N/TileN),W*32,0,stream>>>(c);
    else kernel<1,R,V,W,P,Columns,N,K><<<c.rows*(N/TileN),W*32,0,stream>>>(c);
