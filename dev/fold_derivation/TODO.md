...
| A02 | D10, D12 | PENDING-DEVICE | On the exact admitted product binaries, repeat high-cadence raw-bit/numeric closure for Q4 dense `InterleavedHalf2`, grouped `SeparateHalfPlanes`, shipping AP0 and canonical bc0. Test AP1 and an effective bc1 format in separate same-binary typed diagnostics; neither is a product path and neither may be reported as product coverage. Local type tests do not replace these execution proofs |
