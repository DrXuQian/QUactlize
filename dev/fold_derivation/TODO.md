...
| A04 | D06 | LOCAL-PREBUILT-READY/PENDING-SWEEP | The strict resumable Q4 dense policy-v2 builder and execute-only runner are committed through `b29d53d`. Build its one Q12 payload locally, then sweep all M=1..64 candidates on box. `b87c4d7` adds an independent raw-log adjudicator which emits only measured categorical leaves proven within a conservative 3% regret interval; compiled default and numeric config interpolation are forbidden. This pilot covers only N=1024,K=5120,M=1..64. The other four real families and M>64 remain explicit gaps before A07 |
| A05 | D17 | LOCAL-PREBUILT-READY/PENDING-SWEEP | The strict resumable five-format grouped multi-router bundle and execute-only runner are committed through `b29d53d`. Measure balanced, hot/skewed, sparse/empty, TileM-boundary, and permutation controls. `b87c4d7` independently recomputes per-format minimax/regret from raw logs and groups profiles by the fields the public selector ABI really receives; active/zero/work counts and the offsets histogram are not selector inputs. Enable automatic selection only for a proven <=3% common tactic or proven current-ABI feature classes; otherwise extend the ABI or retain fail-closed explicit selection |
