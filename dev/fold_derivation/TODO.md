...
| A10 initialization-time tuning | IMPLEMENTED / PENDING-DEVICE | `quactlize/runtime/`: bounded measured-family candidates, compile-only image cache, grouped-aware timing cache and device-pointer C parent modules for Q2–Q6 × FQ/SF × dense/grouped. Existing collective types are reused. `tools/run_kpack_warmup_gate.py` is the next small box gate (50 parents, 50 numeric contexts), not a full sweep. See `docs/KPACK_WARMUP.md`. No global 5% guarantee is inferred from bounded local timing. |
| A10 runtime binding | MODULE-ABI-IMPLEMENTED / DEPLOYMENT-PENDING | New `runtime/abi.h` binds AP/delivery/algorithm/S/grid, which config v3/v4 cannot encode. Six published DSOs and llama.cpp remain unchanged. After the module gate, admit selected modules plus the deployment loader and preserve any-M admission until runtime misses have a verified path. |
