...
| A10 deterministic deployment heuristic | PLANNED / NOT-IMPLEMENTED | Calibrate a small route/format-aware selection of complete measured tuples, with shared legality and actual-row grid formulas. Return one tactic; cache/JIT only the chosen parent. Reuse all existing data, retaining explicit performance exceptions; do not require universal unseen-shape 5% proof or fall back to an arbitrary compiled default. Selected-parent reduction must preserve validated coverage/fallbacks. |
| A10 runtime binding | REPRESENTATIVE-MODULE-NUMERICS-PASS / DEPLOYMENT-PENDING | `runtime/abi.h` binds AP/delivery/algorithm/S/grid, which config v3/v4 cannot encode. Real module numerical gate passed; six published DSOs and llama.cpp remain unchanged. Bind the deterministic single-choice heuristic and its selected modules, and preserve any-M admission until runtime misses have a verified path. No automatic online tuning in the default path. |
