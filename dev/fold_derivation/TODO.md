...
| A10 initialization-time tuning | SMALL-DEVICE-GATE-PASS / REAL-SHAPE-RUNNER-READY | Archive `30a86507...`, source contract `e47613f`: 50 modules / 50 contexts / 190 measured candidate-contexts / 250 positive checks / 50 detected zero-low negatives / 10 changed-router cache replays. Ten SF-dense TM8/M9 exclusions are expected. Compile 190.906 s, full harness 254.751 s; warmup calls total 89.476 ms, maximum 13.490 ms. Next runner `tools/run_kpack_warmup_real.py`: 90 representative contexts / 173 parents / 868 candidate-contexts, 85 exact historical incumbents plus 5 explicit new-M transfers; 100-ms soft warmup followed by separate bounded-pool confirmation, full-output oracle and resumable per-case receipts. Twenty representative parents compiled locally in 276.6 s; real-shape device results pending. No full sweep or production kernel change. See `docs/KPACK_WARMUP.md`. |
| A10 runtime binding | SMALL-MODULE-DEVICE-PASS / DEPLOYMENT-PENDING | New `runtime/abi.h` binds AP/delivery/algorithm/S/grid, which config v3/v4 cannot encode. The small module gate passed; six published DSOs and llama.cpp remain unchanged. Admit selected real-shape modules plus the deployment loader, and preserve any-M admission until runtime misses have a verified path. |
