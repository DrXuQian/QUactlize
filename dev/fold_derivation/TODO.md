...
| A10 runtime binding | SINGLE-CHOICE-HOST-PASS / DEVICE-LOADER-PENDING | `runtime/dispatch.py::prepare_selected` binds exact request, complete parent/compiler receipt, device/SDK/kernel and capture state; no profiling or hidden JIT. Existing `runtime/abi.h` encodes AP/delivery/algorithm/S/grid. Next is a short selected-dispatch gate reusing the 55 cached parents, followed by loader/any-M fallback admission. No full sweep or kernel rebuild needed for this host change. Six DSOs and llama.cpp remain unchanged. |
| A10 selected-dispatch gate | BOX-READY / HOST-PASS / DEVICE-NOT-RUN | `tools/run_kpack_selected_gate.py`: fixed 90 contexts / 55 cached parents / one choice per case. No default compilation or online tuning. Live binding/grid, three positive checks, replay/direct raw equality, finite zero-low negative and validation-only 3x5 timing. Expected 270 positive checks / 90 negatives / 25 clean worker exits. Per-case resume; failed groups isolated; cache miss stops before device work. See box command in `docs/KPACK_HEURISTIC_V1.md`. This is not yet llama.cpp loader admission. |
