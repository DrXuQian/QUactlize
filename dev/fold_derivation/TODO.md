...
| A11 existing llama.cpp wiring audit | REVIEWED / TWO-LOCAL-FIXES | At `26955be8a`, reuse buffer ownership, per-format loading, boundary casts and device MoE gather/scatter. Host loader/sidecar tests and Python interoperability pass. Fixed rejected-writer staging ownership/reentry and the disabled-option test-target guard. Sidecar reader/writer still have no model-loader callers; old FQ DSOs still own inference. Operand/device admission and the global borrowed sink need follow-up. See `docs/LLAMA_CPP_KPACK_INTEGRATION_AUDIT.md`. |
| A11 GPU GGUF-to-Kpack producer | IMPLEMENTED / HOST-BYTES-PASS / PPU-BUILD-PASS / DEVICE-PENDING | `quactlize/packing/`: five-format integer-only output-word ownership, canonical metadata, async device API with no allocations/copies/sync. Local producer DSO 126,504 bytes, build 9.355 s; existing GEMM fingerprint unchanged. `tools/run_kpack_pack_gate.py` checks byte equality, guards, negatives, pinned event-ordered D2H and separated timing. It is not an overlap-with-GEMM performance result and is not enabled in llama.cpp. |
| A11 sidecar hit/miss and background persistence | PENDING | Connect the existing reader/writer to the model loader; validated hits bypass raw conversion. Misses pack on device, publish packed-ready, then queue bounded pinned D2H and ordered CPU writes without blocking read-only GEMM. Own input/source identity, events and device-buffer lifetimes through cancellation. Final manifest publication follows completed checks; no unbounded model-sized pinned copy. |
