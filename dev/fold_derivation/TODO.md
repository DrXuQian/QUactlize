...
Latest deployment decision (2026-09-07, after real-shape review): **heuristic +
single-choice cached/on-demand JIT**, not default online tuning. Keep fitting,
candidate timing and evidence validation offline. Reuse measured complete
configurations, keep legality/actual grouped grid resolution source-owned,
and request only residual measurements. Do not turn the old unadmitted blind
Top-1 ranker into a production default by renaming it. Reducing a candidate
table alone does not shrink a DSO: the emitted kernel/link closure must also
be reduced. Published libraries remain unchanged.
...
| A10 measured host rules | LOCAL-PASS / NOT-IN-DSO | `policies/kpack_zw810_compact.{json,hpp}`: 788 leaves, 219 parents / 312 runtime variants, 20 shared trees / 152 exact family guards. Same 2,717 served requests; max training round regret 4.982932%, mean median delta 0.701157%. Near-equal choices merge without relaxing the 5% limit. Python/C++ parity 7,984 queries. See `docs/KPACK_POLICY.md`. |
| A10 targeted merge/boundary validation | RAW-REPLAYED / COMPLETED | Archive `f687bbb2...`: 315 requests x three 11-sample rounds, correctness_repeats=1, 945 raw logs, no numeric/launch failure; 186.26 seconds. Merge targets: 85/320 within both limits, 159 outside, 76 stale grouped fixed grids. New M: 85/110 within both limits. No repeat of the same suite is requested. |
| A10 hybrid heuristic and tactics | LOCAL-CALIBRATED / NOT-IN-DSO | `tools/kpack_tactic_model.py` and `policies/kpack_zw810_tactics.json`: 2,770 exact-context measured cache entries; family hints + calibrated parent/runtime scores produce at most 5 parents / 3 runtime proposals each. Five-fold public-point-grouped internal replay: Top-5 contains a measured within-5% tactic in 2,569/2,985 stable observations; 416 unknown costs and 92 noisy observations stay explicit. Blind Top-1 remains NOT-ADMITTED. Exact grouped Q/grid matches 161,871 raw masks; no fixed neighboring grid is copied. See `docs/KPACK_HEURISTIC_REVIEW.md`. |
| A10 tactic residual challenge | COMPLETE / RAW-REPLAYED | Archive `6e61f4c9...`: 212 requests, 636 raw logs, no numerical/launch/infra failure, all frozen grids available; 307.515 seconds. Shortlist passes both limits for 86/102 cache misses and 107/110 new M; blind Top-1 only 106/212, so it is not deployed. No repeat/new sweep is required for v1 closure. |
| A10 runtime policy v1 | FROZEN / HOST-PASS / NOT-IN-DSO | `policies/kpack_zw810_runtime_v1.{hpp,json}`: exact lookup, no tree/scoring model; 2,982 entries / 247 parents / 509 runtime recipes. 2,889 entries have cross-epoch within-5% evidence; 93 explicit measured exceptions. Latest-epoch bounds hold for 2,968; fourteen remain timing debt. Python/C++ parity 11,928 queries. Unknown inputs explicitly require caller-admitted K-pack fallback. See `docs/KPACK_RUNTIME_V1.md`. |
| A10 tuning experiments | DEVICE-NUMERICS-COMPLETE / NOT-DEPLOYMENT-DEFAULT | Small gate `30a86507...` complete. Real gate `10a00ae9...` matches `ef392a4`: 90 contexts / 173 parents / 868 confirmed candidate-contexts / 2,322 positive checks / 90 detected negatives / 15 bucket checks. Total 740.597 s, cold compilation 353.733 s, warmup calls 7.494 s. Original verdicts 80 within-pool / one gap / nine noise; 89 chosen medians within 5%. Q6 SF grouped boundary missed faster candidate 4 when the budget expired after candidate 2. No numerical failures. Keep as calibration evidence and opt-in tools; no repeated full run. See `docs/KPACK_WARMUP.md`. |
| A10 deterministic deployment heuristic | IMPLEMENTED / HOST-PASS / NOT-IN-DSO | `policies/kpack_zw810_heuristic_v1.{hpp,json}`: recent exact tactics, historical exact tactics, opt-in same-family profile transfer; no fitted tree or online timing. 2,987 exact contexts / 247 effective parents remain explicit data. Recent real-module post-hoc calibration corrects nine historical >5% choices: 90/90 within both limits, max regret 4.153523%, 55 selected parents rather than the 173-parent pool. Python/C++ parity 17,922 queries. Unknown transfers are not performance-admitted. See `docs/KPACK_HEURISTIC_V1.md`. |
| A10 runtime binding | PYTHON-SELECTED-DEVICE-PASS / CPP-LOADER-PENDING | `runtime/dispatch.py::prepare_selected` binds exact request, complete parent/compiler receipt, device/SDK/kernel and capture state; no profiling or hidden JIT. Existing `runtime/abi.h` encodes AP/delivery/algorithm/S/grid. The 90-context selected-dispatch gate is complete. Next: C++ full-identity binding, reusable handles and loader/any-M miss admission; do not run the Python reference on the inference hot path. No repeated full sweep. Six DSOs and llama.cpp remain unchanged. |
| A10 selected-dispatch gate | DEVICE-PASS / RECEIPTS-AND-LOGS-REPLAYED | Archive `174c58a6...`, source contract `05de781`: 90/90 contexts / 55 cached parents, 270 positive checks / 90 finite zero-low negatives / 90 replay and direct raw matches / 25 clean worker exits. Max condition-scaled error 2.76839e-4 < 5e-3; 301.958 s harness wall, zero compilation and online tuning. Plan/source/module keys and prior payload fingerprints match. Thirteen validation timing sets have >5% spread; no same-run alternatives or global performance admission. See `docs/KPACK_SELECTED_GATE_RESULT.json`. No repeat needed; llama.cpp loader and checkpoint validation remain pending. |
| A07 JIT deployment packaging/gate | PENDING | Small C++ selector/module-loader binding + independent GPU conversion DSO + cached compute modules; build small libraries locally. Do not require a new monolithic six-DSO all-config build. Reuse admitted parents and compile missing/ABI-changed modules individually. Verify SF/FQ dense/grouped and model E2E on the final deployment path. Existing six DSOs remain unchanged transitional artifacts. |
| A11 existing llama.cpp wiring audit | REVIEWED / BRANCH-PUSHED | llama.cpp `feat/kpack-gpu-cache` at `9f86a1340`, based on unchanged develop `26955be8a`, reuses buffer ownership, per-format loading, boundary casts and device MoE gather/scatter. Host loader/sidecar tests and Python interoperability pass. Fixed rejected-writer staging ownership/reentry and the disabled-option test-target guard. GPU intake and model-owned sidecar hit/miss are now connected; old FQ DSOs still own inference. Operand/device admission still needs follow-up; global borrowed sink removed. Pushed only to `DrXuQian/llama.cpp`; remote SHA verified, no PR. See `docs/LLAMA_CPP_KPACK_INTEGRATION_AUDIT.md`. |
| A11 GPU GGUF-to-Kpack producer | DEVICE-BYTES-PASS / RESULT-REVIEWED / LOADER-PENDING | Archive `556e61f5...`: exact 15-case union / 75 timing samples, case JSON/summary/console agree, reported DSO hash matches `611ec98c...`. Five formats, N256/K512 with E1/E3 plus N1024/K5120 with E1: 15 initial + 15 post-repeat byte checks, guards and 30 overlap/descriptor rejections pass. Large-anchor pack medians 55.600-121.960 us; first intervals 196.920-430.560 us, not C++ loader latency. Wall 50.431 s includes 49.813 s CPU reference. Event-ordered pinned D2H passes; GEMM overlap, disk persistence and model loading remain pending. Existing GEMM fingerprint unchanged; no repeat of this gate needed. See `docs/LLAMA_CPP_KPACK_INTEGRATION_AUDIT.md`. |
| A11 sidecar hit/miss and background persistence | PPU-MODEL-SMOKE-PASS / RESIDUAL-DEBT | Latest `Lwpxya` on `ced6f241e` passes baseline/cold/hit, all rc=0; raw logs match summaries. Same hash-free 120 tensors (80 Q4_K + 40 Q5_K, grouped E256, 18.125 GiB); hit uploads 120, zero misses/GPU packs. Readiness, new five-format upload preflight and 62 model graph reuses per phase pass. Wall 8.38/22.24/8.54 s; reported load 4.359/4.387/3.909 s. One-model operational coverage, not PPL/KLD or measured physical overlap. Multi-file/custom-loader and broader routes remain pending. No Quactlize DSO change. See handoff/audit. |
| A11 GPU buffer intake and async snapshot seam | BRANCH-PUSHED / PPU-COMPILE-PASS / HOST-ORDERING-PASS | llama.cpp `9f86a1340`: independent GPU producer loader, consumer/producer size checks, reusable whole-expert scratch, input-lifetime wait and separate packed-ready. Dense/grouped enqueue only ready-wait. One bounded-range snapshot API handles full and partial pinned D2H; only the writer waits for completion. Global synchronous sink and redundant whole-copy API removed. Delayed-copy test covers five formats, source reuse, E1000 batching, range/device guards and an injected blocking-copy negative. Model writer now connected; real PPU overlap/model validation remains pending. Source branch pushed; see the handoff. |
| A11 scheduler K-pack buffer admission | BRANCH-PUSHED / NVIDIA-REGRESSION-PASS / PPU-MODEL-SMOKE-PASS | `9f86a1340` aborts at `ggml-backend.cpp:898` on a preallocated `NONE` leaf because `supports_buft` omits K-pack, not because `supports_op(NONE)` rejects it. Exact old-source abort reproduced on RTX 5090; same-device predicate fix passes 21 real-backend reserve/allocation cases, plus wrong-device/type/operator negatives. Four suites each pass 10 repeats. Fix/regression/box smoke entry pushed as `89e5b254d`; `PmwxP9` on `5e632dc04` now passes all three PPU model/cache phases for Q4_K/Q5_K MoE weights. No independent numerical/logit or broad-route admission. No Quactlize DSO rebuild. See the handoff and audit for evidence. |
| A11 grouped C++ dispatcher | DESIGN-GAP / PENDING | New modules require actual `rows_host` for shape/prefix/tile metadata, but llama.cpp has device bounds only. Do not replace actual rows with `n_tokens` or introduce unmeasured per-token D2H/host preparation. Close device-directory binding and any-M/miss handling before enabling the measured selector in the loader. |
| A11 readiness event under graph capture | PPU-PREFLIGHT-PASS / MODEL-REPLAY-PASS | `TXBaN9` rejected flags=0 in capture; `o0ghMa` rejected flags=1 in eager warmup. Fix `5e632dc04` queries capture state and uses the matching flag, without a GPU completion query/wait or D2H dependency. `PmwxP9` passes the real PPU preflight: six transitions, 18 eager checks, 18 graph replays and two missing-wait negatives. Model baseline/cold/hit each exit 0 and report 62 graph reuses. Earlier NVIDIA/host regressions remain supporting evidence. Closure is scoped to this runtime/workload; final dispatcher and broader numerical gates are separate. No Quactlize DSO rebuild. See `docs/LLAMA_CPP_KPACK_HANDOFF.md`. |
| A11 cache startup/publication latency | PPU-MEASURED-IMPROVEMENT / FIRST-WRITE-TAIL-DEBT | `Lwpxya` hit reported load 3.909 s, 10.33% below same-run GPU-pack baseline 4.359 s; wall 8.54 vs 8.38 s. Prior hit was 5.326 s load / 9.98 s wall, but baseline also changed. Host peak RSS 39.14 -> 20.53 GiB. Cold wall remains 22.24 s; writer 12.45 s copy/write + 2.81 s flush, 13.737 s context-to-publication tail. Warm filesystem, one sample per phase, no isolated H2D/overlap attribution or general throughput verdict. No GEMM DSO/plane ABI change. |
| A11 cache-hit original GGUF prefetch | PPU-MODEL-SMOKE-PASS / RESULT-REVIEWED | `bf2877307` included in `ced6f241e`; `Lwpxya` confirms cold prefetch / hit on-demand. Hit RSS falls 18.61 GiB (47.56%) from prior run to 20.53 GiB, versus 21.17 GiB same-run baseline. Cache metadata is inspected before mappings; raw/CPU/declined tensors retain fallback, no_alloc/mlock unchanged. Prior host tests and mmap control pass. Combined run with H2D pipeline, not an isolated performance A/B or cold-disk benchmark. |
| A11 cache-hit pipelined H2D | PPU-DEVICE-BYTES-PASS / MODEL-SMOKE-PASS | `Lwpxya` on `ced6f241e`: exact five-format x E1/E257 upload union passes ten cases, 20 eager reads, 60 graph replays and immediate source reuse. Model hit logs two 8 MiB pinned slots, 120 queued uploads, zero misses/packs. Per-slot waits and buffer-owned source lifetime, ready-event consumption and teardown retained; host early-reuse/synchronous-upload negatives red. No Quactlize DSO change. This closes transport integration for tested cases, not model PPL/KLD or proof of independent H2D speedup. |
| A11 model numerical/performance comparison | EXECUTION-AND-CACHE-REPLAY-PASS / BROADER-PRECISION-PENDING | `CigOEn` archive `7511cc22...`, source `35b74114f`: 10/10 processes rc=0; raw logs, metrics, JSON/TSV and DSO receipts agree. Same 254 scored positions at batch/ubatch 128 and 1; all 120 grouped tensors covered (80 Q4_K + 40 Q5_K). Each K-pack phase has 480/61,440 GEMM calls; ordinary GPU controls zero. Cached/uncached top-token agreement 100%, max KLD 4e-6/4.3e-5, comparable with self-replay. Cached/reference PPL rises 0.2995%/0.0370%, mean KLD 0.003950/0.008010, max 0.268156/0.503132, top-token changes 3/254 and 4/254. Broader accuracy and unprofiled performance remain pending; do not infer a rounding-only cause or a full precision bound. Original SQLite/logprob payloads remain on box. No kernel/DSO change; new C++ dispatcher remains separate. See handoff/audit. |
| A11 extended model validation | BRANCH-PUSHED / HOST-CHECKS-PASS / PENDING-BOX | llama.cpp `d45f8fec2`, same fork feature branch: `run-quactlize-numerical.sh --extended`. Two short cached execution proofs plus six untraced numerical processes and eight separate untraced ABBA model-timing processes. Context 1024 x eight chunks, 8,192 input / 4,088 scored positions per batch 128/1. Reuse cache/DSOs, no GPU repack or CPU reference. 31 local tests, 12 actual-binary CLI-only checks, ten prior raw-log replays pass. Large-run device execution is not newly traced and is labeled accordingly; performance uses model timers with warmup/INFO logging, not process wall or logprob I/O. Two samples per arm are not a guaranteed precision bound. Require 8 GiB free; upload the results archive. No production/kernel change. |
| A11 extended-result follow-up | TOKEN-BATCH-128-NUMERICAL-REVIEWED / PERF-LOG-FIX-PUSHED | `QuHHHY` (`acba469b...`) on `d45f8fec2`: short proof plus three large numerical phases rc=0; 4,088 scored positions, PPL +0.1120%, mean/max KLD 0.002658/0.588895, top-token agreement 98.875%. First perf process rc=0 but zero library timers at verbosity 3. `d25b88a18` uses level 4; native common logger regression reproduces 3=hidden, 4=timers/no-DEBUG. 32 local tests pass. `--performance-only` permits only missing ABBA timings; token-batch-1 remains unstarted. Preserve numerical results; no kernel/DSO change. Both token batches are single-request (`n_seq=1`), not concurrent request batch counts. |
| A11 GSM8K generated-answer accuracy | PILOT-REVIEWED / NO-OBSERVED-ACCURACY-REGRESSION | `EEZKMu` (`cb8b7906...`), llama.cpp `a39917a66`: 128 matched single-request questions, both 122/128 correct, no correctness flips. Raw responses, summary, route and timer receipts replay. One truncated/unparseable case per arm, at different question IDs; 43 identical full outputs. No new device trace or full-benchmark admission. See `docs/KPACK_EXECUTION_FOLLOWUP.md`. |
| A11 decode GEMV and model timing | LOCALLY-COMPILED / DEVICE-PENDING | Five-format dense/compact/indexed GEMV, eight column/warp/split recipes; 560,232-byte execution DSO. 75 local checks including unchanged historical policy replay pass. Box gate adds 81 work points and explicit legacy GEMM core controls; no heuristic/model admission yet. Current model decode remains +29.27% ms/token, not explained away. See `docs/KPACK_EXECUTION_FOLLOWUP.md`. |
| A11 ScaleFirst preparation cost | ANALYZED / IMPLEMENTING / DEVICE-PENDING | Weight-owned GPU expansion, not per-token/pre-request work. Actual 120-tensor Q4/Q5 model adds 3.75 GiB resident scale+zero, with 5.625 GiB minimum first-use logical traffic. Measure prepass, first-use prepass+SF, resident SF and FQ with identical routing; memory admission and graph-safe readiness required. No host bounds readback or dependency on D2H/disk writeback. Details and delivery items 3/4/5 are in `docs/KPACK_EXECUTION_FOLLOWUP.md`. |
| A11 additive grouped device-only ABI | LOCALLY-COMPILED / DEVICE-PENDING | `qk_device_call_v2` sizes from a host upper bound, consumes only GPU offsets, rebuilds shapes/rows/directory per run, and keeps v1 unchanged. Ten measured FQ/SF parents compiled. Gate checks fourteen parent-shape cases, three routing vectors each, v1 raw equality and graph replay. Ordinary bound-grid performance is not automatically inherited from the old exact flat grid. Final C++ selector/llama.cpp integration remains open. |

Numerical follow-up: the first `50079485e` box attempt stopped at corpus
download (rc=4), before any model evaluation. `ced89fc75` adds an offline
`GSM8K_FILE` input (JSONL/JSON/Parquet, first 32 question/answer records);
18 local parser/corpus checks pass. The box uses the existing GSM8K test
Parquet; K-pack PPL/KLD plus actual-kernel-trace results remain pending.
No new GEMM DSO or answer-scoring
claim; all existing execution-evidence requirements remain.
The next CLI-only stop (`--color` unsupported by perplexity) is fixed by
`c7fcaea17`: use `--log-colors off` and preflight exact argv via `--help`.
Old/new option behavior is reproduced with the actual local executable;
six host parser combinations and 18 evidence/corpus tests pass. Box
PPL/KLD and kernel execution are still pending; Asight scope is unchanged.
`F56TMR` then exposed a save-start checker typo: missing `perplexity` in
`calculating perplexity over`. `35b74114f` fixes phase-specific matching;
22 host tests pass, including real C++-format save messages and coverage
negatives. The operator returned a successful reference-save recheck:
batch 128, PPL 1.4282, 12,594 GPU kernels, zero Quactlize grouped GEMMs;
probability payload context 256 / two chunks / 254 scored tokens is complete.
This is only the ordinary GPU reference, not K-pack numeric admission.
The `F56TMR` raw archive was not uploaded. The later `CigOEn` complete
archive is now reviewed: execution/cache replay pass, broader precision
pending as recorded in the row above. The existing fresh-run entry repeats
the reference and does not support resume. No Quactlize DSO rebuild required.
