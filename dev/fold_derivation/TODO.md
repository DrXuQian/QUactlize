...
| A10 targeted merge/boundary validation | PLAN-READY / PENDING-BOX | `tools/run_kpack_policy_validation.py`: 315 requests = 45 original blockers + 160 adjacent-merge requests + 110 new M boundaries; 909 parent/workload pairs / 306 previously compiled parents. No compile path. Three fresh 11-sample rounds, correctness_repeats=1, isolated failures and resumable receipts. Preserve the original timing epoch. |
| A10 interpolation | NOT-ADMITTED | Dense persistent grids now resolve verified capacity/balanced recipes instead of copying fixed grid integers. Grouped row summaries cannot determine exact tile count, so grouped grids remain fixed choices. Earlier holdout results do not validate the new pooled tree. Run the targeted suite; unknown costs and unmeasured queries remain proposals, not a universal 5% guarantee. |
