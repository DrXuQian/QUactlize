...
| Interleaved `dB` | Generic B stride `dB` is accepted | The interleaved branches reject noncanonical markers, select `ptr_B + l*(N*K*bits/8)` through `mixed_packed_byte_expert_base`, then build a separate inner layout whose strides are all logical codes | Canonical interleaved byte layout and expert pitch `N*K*bits/8`; inner descriptor geometry remains in logical codes | Caller supplies row padding, a different expert pitch, or any otherwise noncanonical `dB`; or code combines the byte L pitch and logical inner strides in one raw tensor | Shipping adapters were already canonical. L129 rejects every noncanonical marker; the sub-byte unit audit split the byte expert-base selection from the pure logical inner layout so generic indexing cannot reinterpret mixed units | Hold pointer/shape fixed and perturb each `dB` component; all three arms reject. Source plants reject restoring a mixed logical-code/byte stride tuple. Independent anchors are the explicit `N*K*bits/8` byte pitch and the existing xplane placement roundtrip | **L129 + unit audit FIXED** (format remains intentionally canonical) |
