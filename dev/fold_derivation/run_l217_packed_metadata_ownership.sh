...
main() {
  local repo out compiler positive_rc negative_rc
  ...
  if [ "$positive_rc" -ne 0 ] || [ "$negative_rc" -eq 0 ]; then
    printf '[l217-runner] FAIL: positive=%d legacy=%d\n' "$positive_rc" "$negative_rc" >&2
    return 1
  fi
  grep -Fqx \
