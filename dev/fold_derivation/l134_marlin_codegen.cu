...
CUTLASS_HOST_DEVICE constexpr auto l134_make_params(
    ...
    unsigned int blocks_per_cu = 1) {
#if defined(L134_IGNORE_BLOCKS_PER_CU_PLANT)
  blocks_per_cu = 1;
#endif
  ...
  return L134Scheduler::make_params_for_problem_shape(
      scheduler_shape, cu, nullptr, blocks_per_cu);
#endif
}
...
constexpr auto l134_decode_b2 =
    l134_make_params(cute::make_shape(1, 4096, 4096, 1), 72, 2);
...
static_assert(l134_decode_b2.grid_blocks_ == 144 &&
              l134_decode_b2.iters_per_block_ == 8 &&
              l134_decode_b2.active_blocks_ == 128,
              "real dense Cfg must lower the explicit B=2 launch cohort");
...
l134_marlin_constexpr_witness[35] = {
  ...
  L134Kernel::MaxThreadsPerBlock,
  l134_decode_b2.grid_blocks_, l134_decode_b2.iters_per_block_,
  l134_decode_b2.active_blocks_,
};
...
extern "C" __global__ void l134_marlin_runtime_probe(
    ...
    unsigned long long l, unsigned long long cu, unsigned int blocks_per_cu,
    unsigned long long* out) {
  auto params = l134_make_params(
      cute::make_shape(int(m), int(n), int(k), int(l)), cu, blocks_per_cu);
