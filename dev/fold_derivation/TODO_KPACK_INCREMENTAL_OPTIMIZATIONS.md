...
- GATE INFRASTRUCTURE: typed-device-query repair is published at artifact `11d0f34`; the subsequent missing canonical producer symbol is a stale dependency in the runner, not a numerical result. Use `run_kpack_decode_io_device_fix_box.sh`, now pulling the existing `kpack-fusion-v1` paired producer with an early four-export/canonical/size check. No GEMM rebuild for the producer-path correction; PPU endpoint/chain admission remains pending.
- BOX GATE READY: dense M1..8 explicit F32/BF16 storage, unchanged FP16 compute and original FP16 ABI; conversion in the A load and output epilogue/reducer. Existing selected SIMT/TC recipe retained, no second tuner. Fused MoE prepare/finish extended to top8 tokens5..8 (40..64 rows). 27 typed parents, five controls and two SIMT-stage binaries compiled locally; [contract/command](../../docs/KPACK_DECODE_IO.md). PPU numerical/trace and model-speed admission are still pending.
- QUEUED AFTER endpoint implementation handoff (user update, 2026-09-14): `/root/kpack-cost-multi.i8l9xA.results.tgz` is uploaded. Audit the returned component timings and then update the full heuristic, including measured Split-K reducers and large-M full-dequant candidates; small M does not use full dequant. Do not mix this policy change into the endpoint gate or remeasure valid results without cause.
...
| Full BF16 packed-code exchange | PPU reviewed:170 timed configs plus two smokes, six ACU profiles; all34 best medians improve, three new-reader shared-conflict totals are0 | All12 Q4/Q5 E256 domains select c11; retain per-shape choices for dense/E8. [v4 results](../../docs/KPACK_FULL_PACKED_EXCHANGE.md); no production route change |
| Independent component composition | COMPLETE:20 dense cuBLAS +24 grouped Python-JIT DeepGEMM costs rebased onto exact-output-matching v4 expansion; Q4/Q5 grouped sum reductions11.25--13.04% /7.65--9.45% | [44-point table](../../docs/KPACK_PREFILL_COMPONENT_COSTS.md). Missing FQ/SF GEMM costs stay null; sum is not measured E2E |
| Matched FQ/SF prefill GEMM components | BOX READY:44 workload/token points x2 routes,88 measurements through the production heuristic; all currently S1,12 compiled parents | [Executable commands](../../docs/KPACK_PREFILL_SELECTED_MEASUREMENT.md). No online tuning or dequant/BF16 resweep. Preserve complete-output timing and exclude SF prepass. Historical winner challenges remain a separate follow-up if needed; this run does not prove global config optimality |
| Standalone Split-K reducer costs | BOX READY:48 deduplicated dense/grouped M/N/S2/S4/S8 points; exact production reducer types, no copied kernel body | Same command entry with `reducer`; synthetic FP32 partials, rotating cold ring, independent ordered-sum/negative/guard checks. Diagnostic only: do not add to complete GEMM timing or force S>1 for the S1 prefill choices |
...
| Single-token indexed SIMT | PPU OPERATOR PASS; [matched TC comparison reviewed](../../docs/Q4_MOE_COMPARE_RESULTS_20260913.md). All12 token1 cases favor SIMT; shared-A calls7.773..16.613us versus complete TC20.268..27.592us. One launch resolves GPU IDs/A/weights/output with no standalone gather/scatter. Token1/top8 remains eight separate expert rows. No MoE raw-reference or model-level parity claim. |
| Multi-token indexed SIMT | PPU NUMERIC PASS token1..8, both activation conventions, padded strides, empty/clustered experts and changed GPU IDs on graph replay. Previous18 noisy timing cases have now been reconfirmed in the matched TC gate. All96 dispersed cases favor SIMT; clustered token8 favors TC in five of six N/K families. ACU shows much higher SIMT on-chip reads/decode despite similar DRAM bytes. Partial-collision expert-local rows2..7 remain unmeasured; do not generalize the two extreme routers into a token-only rule. |
...
| Unified sweep and heuristic | [MoE same-cohort gate BOX REVIEWED](../../docs/Q4_MOE_COMPARE_RESULTS_20260913.md):108/108 cases,3,276 numeric screens,2,592 confirmations,55,260 samples,12 ACU reports/64 kernels. SIMT wins98; TC wins10, all clustered-token8. All96 dispersed cases favor SIMT. Prior18 noisy SIMT cases reconfirmed; two TC token4 spans remain noisy but far from route ties. Development64-row TC adapter numerically passes this gate; shipping fused ABI still stops at32 rows. NEXT: broader prior-shape sweep and compact policy, partial-collision rows2..7 boundary, no CPU histogram assumption, retain old dense TM8/S4 winner and chain fusion. |
| TC indexed preparation overhead | MEASURED DEBT, separate from the SIMT reader. Matched ACU token1 prepare8.94..9.41us, only0.009..0.020MB DRAM input: routing/A/metadata, no B copy. Current indexed entry repeats ranks for K-chunk x row CTAs. Development token8 fallback gather10.25..14.77us. Keep costs in comparisons; optimize/admit separately without removing successful timing evidence. |
