...
| Matched FQ/SF prefill GEMM components | NEXT:44 workload/token points x2 routes,88 measurements before bounded tactic challenges; use the same device, canonical bytes, expert rows and rotating-weight timing | Do not rerun dequant/cuBLAS/DeepGEMM. Keep historical winning configs as challenges; time Split-K completion, exclude already measured SF prepass, preserve numerical contracts and list adapter costs separately |
