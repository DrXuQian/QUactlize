...
// every declared row and its first named exclusion; --header emits the exact
// admitted-row registry consumed by the standalone benchmark build.
#include <array>
#include <cstdint>
#include <cstdio>
#include <cstring>
...
char const* load_token(mt::MarlinLoadKindPPU load) {
  switch (load) {
    case mt::MarlinLoadKindPPU::CpAsync: return "CP_ASYNC";
    case mt::MarlinLoadKindPPU::Aiu: return "AIU";
  }
  return "UNKNOWN";
}
...
uint64_t admitted_count() {
  ...
  mt::for_each_declared([&](mt::MarlinTacticPPU c) {
    count += mt::admitted(c) ? 1 : 0;
  });
  return count;
}

void print_header() {
  std::puts("// GENERATED by dev/fold_derivation/emit_marlin_tactic_space.cpp --header; do not edit.");
  std::puts("// Schema: X(TM,TN,TK,WM,WN,WarpK,ST,LoadToken,B).");
  std::puts("// B carries the dispatch body through the one admitted-row authority.");
  std::printf("#define MARLIN_STANDALONE_CFG_ROWS %llu\n",
              static_cast<unsigned long long>(admitted_count()));
  std::puts("#define MARLIN_STANDALONE_CFG_LIST(X, B) \\");

  uint64_t emitted = 0;
  uint64_t const total = admitted_count();
  mt::for_each_declared([&](mt::MarlinTacticPPU c) {
    if (!mt::admitted(c)) return;
    ++emitted;
    std::printf("  X(%d,%d,%d,%d,%d,%d,%d,%s,B)%s\n",
                c.tm, c.tn, c.tk, c.wm, c.wn, c.warp_k, c.stages,
                load_token(c.load), emitted == total ? "" : " \\");
  });
}
...
int main(int argc, char** argv) {
  ...
  bool const header = argc == 2 && std::strcmp(argv[1], "--header") == 0;
  if (argc > 2 || (argc == 2 && !all && !header)) {
    std::fprintf(stderr, "usage: %s [--all|--header]\n", argv[0]);
    return 2;
  }
