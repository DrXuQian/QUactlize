...
| T15 | Cross-call weight prefetch | Two bounded experiments implemented: target benefit after prefetch, and current-call interference with concurrent next-weight prefetch. Reuse Q4 TM8/S4 and Q5 TM8/S1; one small auxiliary library, no production changes or hard CU partition | Unprofiled graph/event A/B, real timeline concurrency and cache-pressure sensitivity; ACU kernel replay must not erase prefetch state. [Protocol](KPACK_PREFETCH_EXPERIMENT.md) |
