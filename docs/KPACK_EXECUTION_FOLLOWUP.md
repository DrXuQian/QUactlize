...
| PPU GEMV / FQ / SF comparison | Initial box run failed at pair API lookup, before timing; runner repaired, box rerun pending | [Five-case prebuilt gate](KPACK_GEMV_FQ_SF.md): correct published pair DSO, actual export checks and 240 host queries. Three small decode anchors plus two larger-weight M1 controls; old/new SIMT, selected FQ/SF, common F32 endpoints, resident cores, SF prepass and ACU. No production selection or binary change |
