...
## Current K-pack policy (2026-09-02)

The fully-quantized K-pack route now has an embedded, generated measured table.  It never profiles at inference
time. Dense selection order is:

1. a nonempty compiled config name supplied by the caller;
2. an exact measured K-pack point;
3. the existing compiled default.

Dense measurements are keyed by `(qtype, m, n, k)`; a neighbouring M or an
unmeasured N/K pair is a miss rather than an interpolation. Grouped automatic
selection is deliberately not admitted yet. Its kernel schedule depends on
the full per-expert row histogram, while the existing public launch ABI can
observe only `(total_rows,max_rows,experts)`. Many unmeasured router
distributions share those aggregates. Until the route identity is either
carried by the ABI or measured across a justified distribution envelope,
grouped null config retains the compiled default and explicit names remain
available.

The source is generated by `tools/generate_fq_kquant_measured_policy.py` into
`quactlize/include/ppu_kquant_measured_policy_data.inc`. Generation reads and
hashes every file named by the result authority, binds every complete candidate
set, and recomputes the maximum 3% within-K-pack regret. The input evidence contains
64 N/K families, 148 measured intervals and 13 exact grouped signatures; only
its 44 dense families and 110 dense intervals currently drive an automatic
production route. Its evidence records
`unverified-sdk`: all 41,262 raw measurements are numerically clean and authority-complete, while the box SDK's
compiler/inspector hashes were allowed to differ from the build SDK.  A verified-SDK rerun can upgrade that
provenance without changing the runtime contract.

This table answers only **which K-pack tactic to use**.  It does not claim K-pack beat Xplane.  The format
comparison remains visible technical debt (roughly 0.5--0.8% aggregate/mean cost and about 7.9% worst local
regression in this campaign), accepted separately in exchange for one prefill/decode resident format.
