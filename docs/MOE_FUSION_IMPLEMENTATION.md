...
| NCU-guided SIMT reader | Development N2 candidate reduces Q4/Q5 full calls about 28-29% on 5090 and 38-42% on 5070; five-format 256+360-cell gates pass. Q8 24-recipe screen finds large-shape gains but a small indexed regression | Port to PPU and run bounded numerical/ACU gate; retain shape-specific old-reader choices. No production recipe change; Q4/MMVQ performance gap remains |
| Historical Xplane SIMT gap | [Matched Q4 5090 A/B](Q4_SIMT_XPLANE_KPACK_5090_AB.md): K-pack N2 is 89-93% slower for >L2 M1/N4096/K2048-4096; S1 is also slower; all numeric gates pass | Compare matched accumulation precision and A reuse/load topology. Do not infer SIMT parity from the older GEMM comparison; no format rollback or selector promotion from this NVIDIA-only result |
| SIMT ALU/load-topology attribution | [5070 NCU](Q4_SIMT_XPLANE_KPACK_NCU.md), fixed 5090 recipes: cold K4096 Xplane/K-pack DRAM=80.14/43.98%, warp instructions=2.02/11.66M; N2 SM=ALU-heavy 76.38%, FMA=20.97%, excessive global sectors=68% | Attribute excessive sectors and ALU work to actual load/decode sites; separate precision and activation reuse. Reducer is only 1.89 us here; no PPU or automatic-policy change |
