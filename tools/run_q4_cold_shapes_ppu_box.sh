...
(
    ...
    trap finish EXIT
    trap 'printf "Q4_COLD_SHAPES_BOX FAIL stage=%s line=%s rc=%s\n" "$stage" "$LINENO" "$?" >&2' ERR
    ROOT=$(git -C "$(dirname "${BASH_SOURCE[0]}")/.." rev-parse --show-toplevel)
    test -n "$ROOT" && test -f "$ROOT/dev/gemv_ppu/run_cold_shapes.py"
    cd "$ROOT"
    ...
    "$PYTHON" -c 'from pathlib import Path; from dev.gemv_ppu.cold_shapes import verify; verify(Path("prebuilt/ppu0010/q4-cold-shapes-v1"),Path("prebuilt/ppu0010/q4-h800-port-v1"),Path("prebuilt/ppu0010/q4-simt-ab-v1")); print("Q4_COLD_SHAPES_PACKAGE PASS shapes=6 qtype=Q4_K production=UNCHANGED")'
    if [[ -n ${RESUME_RUN:-} ]]; then
        ...
        RUN=$(mktemp -d "$RESULT_DIR/q4-cold-shapes-ppu.XXXXXX")
...
for n, k in SHAPES:
    ...
    if path.exists():
        read_fixture(path)
        print(f"Q4_COLD_SHAPES_FIXTURE reuse={path.name}", flush=True)
    else:
...
PY
    ...
    stage=compare
    printf 'Q4_COLD_SHAPES_BOX run=%s qtype=Q4_K shapes=6 M=1 cache=ROTATING_ONLY arms=xplane,raw-reference,kpack-current,kpack-c8\n' "$RUN"
    printf 'PREBUILT=1 compile=NONE JIT=NONE rounds=6 samples=15 timing_cells=144 first_launch=EXCLUDED\n'
    "$PYTHON" -u dev/gemv_ppu/run_cold_shapes.py --sdk "$SDK" \
