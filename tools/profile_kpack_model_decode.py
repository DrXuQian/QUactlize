...
def profile_plan(selection, kernels, log, helpers):
    ...
    if not rows:
        raise ValueError('completed trace contains no selected M1 SIMT call')
    ...
    # Three costly dense shapes, two routed shapes and prepare suffice for
    # diagnostics; this is not another config sweep or a timing authority.
    dense = sorted((r for r in rows if r['point'].get('mode')==0),
                   key=lambda r:r['point']['n']*r['point']['k'],reverse=True)[:3]
    grouped = [r for r in rows if r['point'].get('mode')==2][:2]
    prepares = [r for r in rows if r['point']['kind']=='prepare'][:1]
...
def collect(a):
    ...
    for directory in sorted(a.trace.iterdir()):
        ...
        print(f'KPACK_MODEL_ACU_PLAN model={directory.name} reports={len(plan)} compile=NONE',flush=True)
        for i,job in enumerate(plan):
            ...
            try:
                ...
                if not any(re.sub(r'\s+','',name) in normalized for name in job['observed_symbols']):
