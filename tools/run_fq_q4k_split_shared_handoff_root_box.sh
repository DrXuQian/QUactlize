#!/usr/bin/env bash
# Second-stage root bundle: keep a CTA barrier for every shared round-trip arm
# and change exactly one data-handoff seam at a time.
set -uo pipefail

main() {
  local root workspace_root sha short stamp out jobs repeats
  local full generated arm defs def build_dir build_log binary
  local direct_log probe_log build_rc direct_rc probe_rc

  root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)" || return 2
  workspace_root="$(realpath -e /workspace)" || return 2
  if [ -n "${PPU_DEFS:-}" ] || [ -n "${PPU_EXTRA_DEFS:-}" ]; then
    echo '[fq-shared-handoff-root] FAIL: ambient defines change exact arms' >&2
    return 2
  fi
  sha="$(git -C "$root" rev-parse HEAD)" || return 2
  short="${sha:0:8}"
  stamp="$(date -u +%Y%m%dT%H%M%SZ)" || return 2
  out="$(realpath -m -- "${OUT:-/workspace/quactlize-fq-q4k-shared-handoff-root-${short}-${stamp}-$$}")" || return 2
  case "$out" in
    "$workspace_root"/*) ;;
    *) echo "[fq-shared-handoff-root] FAIL: OUT must be below /workspace: $out" >&2; return 2 ;;
  esac
  if [ -e "$out" ]; then
    echo "[fq-shared-handoff-root] FAIL: refusing to overwrite $out" >&2
    return 2
  fi
  jobs="${JOBS:-16}"
  repeats="${PROBE_REPEATS:-512}"
  case "$jobs" in
    *[!0-9]*|0) echo '[fq-shared-handoff-root] FAIL: JOBS must be positive' >&2; return 2 ;;
  esac
  case "$repeats" in
    *[!0-9]*|0) echo '[fq-shared-handoff-root] FAIL: PROBE_REPEATS must be positive' >&2; return 2 ;;
  esac
  mkdir -p "$out/generated/full" "$out/generated/closure" "$out/results" || return 2

  git -C "$root" show \
    "$sha:dev/fold_derivation/l223_fq_splitk_shared_epilogue_layout.expected.txt" \
    >"$out/results/l223-committed-evidence.log" || {
      echo '[fq-shared-handoff-root] FAIL: committed L223 evidence missing' >&2
      return 2
    }
  grep -Fqx \
    '[l223] PASS: exact legacy R2S/S2R map plus value-coordinate and owner negatives' \
    "$out/results/l223-committed-evidence.log" || {
      echo '[fq-shared-handoff-root] FAIL: committed L223 oracle is not admitted' >&2
      return 2
    }

  python3 -B "$root/tools/select_fq_split_timing_closure.py" --self-test || return 2
  python3 -B "$root/tools/check_fq_split_shared_handoff_root.py" --self-test || return 2
  python3 -B "$root/ci/check_fq_split_shared_sync_root.py" || return 2
  python3 -B "$root/tools/gen_fully_quantized_splitk_producer_units.py" \
    --qtype 12 --artifact-tk 64 --bchunk 0 --tile-m-filter 8 \
    --per-unit 1 --out-dir "$out/generated/full" || return 2
  full="$out/generated/full"
  generated="$out/generated/closure"
  python3 -B "$root/tools/select_fq_split_timing_closure.py" \
    --source-dir "$full" --out-dir "$generated" || return 2

  for arm in cta-baseline discard-roundtrip pre-r2s-cta disjoint-storage \
             identity-convert scalar-r2s scalar-s2r scalar-both; do
    case "$arm" in
      cta-baseline)
        defs='PPU_SPLITK_SHARED_SYNC_POLICY=3' ;;
      discard-roundtrip)
        defs='PPU_SPLITK_SHARED_SYNC_POLICY=3 PPU_SPLITK_SHARED_PROBE_DISCARD_GMEM=1' ;;
      pre-r2s-cta)
        defs='PPU_SPLITK_SHARED_SYNC_POLICY=3 PPU_SPLITK_SHARED_PROBE_PRE_R2S_CTA=1' ;;
      disjoint-storage)
        defs='PPU_SPLITK_SHARED_SYNC_POLICY=3 PPU_SPLITK_SHARED_PROBE_DISJOINT_STORAGE=1' ;;
      identity-convert)
        defs='PPU_SPLITK_SHARED_SYNC_POLICY=3 PPU_SPLITK_SHARED_PROBE_IDENTITY_CONVERT=1' ;;
      scalar-r2s)
        defs='PPU_SPLITK_SHARED_SYNC_POLICY=3 PPU_SPLITK_SHARED_PROBE_SCALAR_R2S=1' ;;
      scalar-s2r)
        defs='PPU_SPLITK_SHARED_SYNC_POLICY=3 PPU_SPLITK_SHARED_PROBE_SCALAR_S2R=1' ;;
      scalar-both)
        defs='PPU_SPLITK_SHARED_SYNC_POLICY=3 PPU_SPLITK_SHARED_PROBE_SCALAR_R2S=1 PPU_SPLITK_SHARED_PROBE_SCALAR_S2R=1' ;;
      *) return 2 ;;
    esac
    build_dir="$out/build-$arm"
    build_log="$out/results/$arm-build.log"
    mkdir -p "$build_dir" || return 2
    (cd "$root" && PPU_BUILD_DIR="$build_dir" PPU_ARCHS=ppu0010 JOBS="$jobs" \
      PPU_DEFS="$defs" TARGET=test_fully_quantized_internal_sweep \
      FQ_SWEEP_GENERATED_DIR="$generated" FQ_SWEEP_QTYPE=12 \
      FQ_SWEEP_ARTIFACT_TK=64 FQ_SWEEP_BCHUNK=0 \
      FQ_SWEEP_PACKED_FORMAT=0 ./build.sh) >"$build_log" 2>&1
    build_rc=$?
    if [ "$build_rc" -ne 0 ]; then
      echo "[fq-shared-handoff-root] FAIL: $arm build rc=$build_rc" >&2
      tail -180 "$build_log" >&2
      echo "[fq-shared-handoff-root] artifacts=$out" >&2
      return "$build_rc"
    fi
    for def in $defs; do
      grep -Fq -- "-D$def" "$build_log" || {
        echo "[fq-shared-handoff-root] FAIL: $arm define missing: $def" >&2
        return 2
      }
    done
    if grep -Fq -- '-DPPU_PACKED_METADATA_OWNER_ONLY' "$build_log"; then
      echo "[fq-shared-handoff-root] FAIL: $arm contains metadata override" >&2
      return 2
    fi

    binary="$build_dir/ppu_targets/test_fully_quantized_internal_sweep"
    if [ ! -x "$binary" ] || [ -L "$binary" ]; then
      binary="$(grep -m1 '^built: ' "$build_log" | cut -d' ' -f2-)"
    fi
    if [ -z "$binary" ] || [ ! -x "$binary" ] || [ -L "$binary" ]; then
      echo "[fq-shared-handoff-root] FAIL: $arm binary missing: $binary" >&2
      return 2
    fi

    direct_log="$out/results/$arm-direct.log"
    probe_log="$out/results/$arm-probe.log"
    "$binary" --shape=1x1024x5120 --iterations=1 \
      --correctness-repeats="$repeats" --tm8-max-m=8 --bc-mode=skip \
      >"$direct_log" 2>&1
    direct_rc=$?
    "$binary" --shape=1x1024x5120 --iterations=1 \
      --correctness-repeats="$repeats" --tm8-max-m=8 --bc-mode=skip \
      --split-workspace-probe >"$probe_log" 2>&1
    probe_rc=$?
    if { [ "$direct_rc" -ne 0 ] && [ "$direct_rc" -ne 1 ]; } || \
        [ "$probe_rc" -ne 0 ]; then
      echo "[fq-shared-handoff-root] FAIL: $arm rc direct=$direct_rc probe=$probe_rc" >&2
      tail -100 "$direct_log" >&2
      tail -120 "$probe_log" >&2
      echo "[fq-shared-handoff-root] artifacts=$out" >&2
      return 2
    fi
    printf 'FQ_SHARED_HANDOFF_BINARY arm=%s defs=%q sha256=%s direct_rc=%d probe_rc=%d repeats=%d\n' \
      "$arm" "$defs" "$(sha256sum "$binary" | awk '{print $1}')" \
      "$direct_rc" "$probe_rc" "$repeats"
  done

  local -a verdict_args=()
  for arm in cta-baseline discard-roundtrip pre-r2s-cta disjoint-storage \
             identity-convert scalar-r2s scalar-s2r scalar-both; do
    verdict_args+=(
      "--$arm-direct" "$out/results/$arm-direct.log"
      "--$arm-probe" "$out/results/$arm-probe.log"
    )
  done
  python3 -B "$root/tools/check_fq_split_shared_handoff_root.py" \
    "${verdict_args[@]}" | tee "$out/results/verdict.log" || {
      echo "[fq-shared-handoff-root] UNADJUDICATED artifacts=$out" >&2
      return 2
    }
  sha256sum "$generated/manifest.json" "$out"/results/*.log \
    >"$out/results/authority.sha256" || return 2
  echo "[fq-shared-handoff-root] ROOT_CAUSE_COMPLETE sha=$sha artifacts=$out"
}

main "$@"
