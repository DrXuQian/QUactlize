#!/usr/bin/env bash
# Exact epilogue-only bisection for the grouped Q4 K-pack TM8 failure.
# It builds one production-type binary and compares the same M=9,N=64 output:
#   control: TN32/WN32, two one-warp CTAs on N
#   subject: TN64/WN16, one four-N-warp CTA
...
main() {
  ...
  local control m8_control subject control_owner subject_owner
  control="$(grep '^FQ_M8_EPILOGUE_TOPOLOGY arm=tn32-wn32-control ' "$RUN_LOG" || true)"
  m8_control="$(grep '^FQ_M8_EPILOGUE_TOPOLOGY arm=tn64-wn16-m8-control ' "$RUN_LOG" || true)"
  subject="$(grep '^FQ_M8_EPILOGUE_TOPOLOGY arm=tn64-wn16-subject ' "$RUN_LOG" || true)"
  control_owner="$(grep '^FQ_M8_EPILOGUE_FIRST_TILE_OWNERSHIP arm=tn32-wn32-control ' "$RUN_LOG" || true)"
  subject_owner="$(grep '^FQ_M8_EPILOGUE_FIRST_TILE_OWNERSHIP arm=tn64-wn16-subject ' "$RUN_LOG" || true)"
  if [ -z "$control" ] || [ -z "$m8_control" ] || [ -z "$subject" ] || \
     [ -z "$control_owner" ] || [ -z "$subject_owner" ]; then
  ...
  if [[ "$control" != *' packed_metadata=1 '* ]] || \
     ...
     [[ "$subject" != *' packed_metadata=1 '* ]]; then
    ...
    verdict=CONTROL_DIRTY
  elif [[ "$subject_owner" == *' first8_bad=0/512 guard_bad=0 finite_bad=0 '* ]] && \
       [[ "$subject_owner" == *' ILLEGAL_ROW8_WRITE' ]]; then
    verdict=TN64_WN16_EPILOGUE_CAUSAL
  elif [[ "$subject" == *' positive_bad=0/576 '* ]] && \
       [[ "$subject" == *' negative_oracle_bad=0/576 '* ]] && \
       [[ "$subject" == *' observed_red=432 expected_red=432 '* ]] && \
       [[ "$subject" == *' cohort_red=[0,144,144,144] EXPECTED_RED' ]]; then
    verdict=EPILOGUE_EXCLUDED
  else
    verdict=TN64_WN16_EPILOGUE_OTHER_SEAM
  fi
