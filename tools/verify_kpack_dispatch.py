...
PACK_MODEL_EXPORTS = frozenset('quactlize_ppu_' + name for name in (
    'kpack_canonical_arrangement_v1 kpack_sizes_for_arrangement_v1 '
    'prepare_fully_quantized_dev_for_arrangement_v2 prepare_gate_up_dev_for_arrangement_v1').split())
...
def pack_paths(root, receipt, *, model_formats=None):
    ...
    if model_formats is not None:
        require_exports(root / 'pack/libquactlize_ppu_pack.so', PACK_MODEL_EXPORTS)
        if not set(model_formats) <= set(build.get('formats', [])):
            raise ValueError('model packer does not support every compute format')
...
def verify(root, *, sdk=None):
    ...
    if 'pack' in m:
        pack_paths(root, m['pack'], model_formats=m.get('compute_contract', {}).get('formats'))
