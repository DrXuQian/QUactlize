...
(
    ...
    trap finish EXIT
    trap 'printf "Q4_SMALL_BOX FAIL stage=%s line=%s rc=%s\n" "$stage" "$LINENO" "$?" >&2' ERR
    ROOT=$(git -C "$(dirname "${BASH_SOURCE[0]}")/.." rev-parse --show-toplevel)
    test -n "$ROOT" && test -f "$ROOT/dev/gemv_ppu/run_small_latency.py"
    cd "$ROOT"
    ...
    "$PYTHON" -c 'from pathlib import Path; from dev.gemv_ppu.small_latency import verify; p=Path("prebuilt/ppu0010"); verify(*(p/name for name in ("q4-small-latency-v1","q4-reader-followup-v1","q4-reader-reuse-v1","q4-config-sweep-v1","q4-cold-shapes-v1","q4-h800-port-v1","q4-simt-ab-v1"))); print("Q4_SMALL_PACKAGE PASS shapes=2 candidates=80 qtype=Q4_K production=UNCHANGED")'
    if [[ -n ${RESUME_RUN:-} ]]; then
        ...
        RUN=$(mktemp -d "$RESULT_DIR/q4-small-latency-ppu.XXXXXX")
...
from dev.gemv_ppu.small_latency import SHAPES
...
for n, k in SHAPES:
    ...
    if path.exists():
        read_fixture(path)
        print(f"Q4_SMALL_FIXTURE reuse={path.name}", flush=True)
    else:
...
PY
    ...
    stage=compare
    printf 'Q4_SMALL_BOX run=%s qtype=Q4_K shapes=2 M=1 cache=ROTATING_ONLY prior_four_shapes=CLOSED_NOT_RERUN\n' "$RUN"
    printf 'PREBUILT=1 compile=NONE JIT=NONE candidates=80 screen_cells=86 confirm_cells=72 profiles=10 rounds=6 confirm_samples=15 ref_limit_pct=5 first_launch=EXCLUDED\n'
    "$PYTHON" -u dev/gemv_ppu/run_small_latency.py --sdk "$SDK" \
