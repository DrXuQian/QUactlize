...
(
    ...
    trap finish EXIT
    trap 'printf "Q4_BLOAD_BOX FAIL stage=%s line=%s rc=%s\n" "$stage" "$LINENO" "$?" >&2' ERR
    ROOT=$(git -C "$(dirname "${BASH_SOURCE[0]}")/.." rev-parse --show-toplevel)
    test -n "$ROOT" && test -f "$ROOT/dev/gemv_ppu/run_bload.py"
    cd "$ROOT"
    ...
    if [[ -n ${RESUME_RUN:-} ]]; then
        ...
        RUN=$(mktemp -d "$RESULT_DIR/q4-bload-ppu.XXXXXX")
