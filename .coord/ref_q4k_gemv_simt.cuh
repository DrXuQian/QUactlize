// USER-SUPPLIED REFERENCE KERNEL — verbatim, not authored by claude, not placed in the source tree.
// Handed over 2026-08-14 for INBOX 171. codex decides where this lands (benchmarks/ or tests/), what the
// PPU-asm arm looks like, and what the harness around it is. Nothing here has been reviewed or built.
//
// PROVENANCE: the Q4_K SIMT GEMV from the paper PDF the user shared earlier this session
// (~/.claude/uploads/.../3c34652c-q4kgemv.pdf). NVIDIA PTX mnemonics (prmt/lop3/sub.f16x2/fma.rn.f16x2),
// so it compiles for sm_120 as-is; the PPU arm needs ppu.prmt.b32 / ppu.lop3.b32 / ppu.sub.f16x2 /
// ppu.fma.rtte.f16x2, which is the sequence actlize's MixGemmNumericArrayConverter<half_t,int4b_t,8>
// already emits (4 lop3 + 2 sub + 2 fma per 8 codes — confirmed against the acu instruction mix).
