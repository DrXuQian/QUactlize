...
## 178 — **撤回 177。真正要做的是 split-K collective:把 (1,32768,512) 相对 (1,4096,4096) 的 2.57× 固化进 kernel**

### 零、先撤回 177

177 提的 last-peer-wins 只改**归约方式**,对现有 P≈2.25 生效,**把 S 提不到 8**。用户要的是**分解本身**变成 S 路。归约方式是第二个、正交的决定,不在本条。**177 不要执行。**

### 一、用户在 PPU 上的实测(冷缓存,两边各自 sweep 的最优,同 gs、同权重)

    形状                时间      GB/s     %HBM(/2766)   权重/核/周期
    (1, 32768, 512)     7.4 µs    1169      42.3%          18.5
    (1,  4096, 4096)   17   µs     509      18.4%           8.06
                        ──────                              ────
                        2.30×                               2.30×

**(2026-08-14 用户更正:(4096,4096) 是 17 µs 不是 19,本条的比值/预算/判据已按 17 重算。)**
17 µs / 18.4% / 8.06 与 standalone Marlin 那次 acu(16.96 µs / 18.5% / 8.08)几乎重合 —— 值得留意

参照:PPU 上参考 SIMT GEMV 是 **22.8** 权重/核/周期(INBOX 132,口径未知)。**18.5 已到它的 81%。**

**⚠ 两台机器方向相反,这是本条最重要的方法学警告:**

    5090 冷(我实测)   (32768,512) 10.451 µs / 50.7%   ← **最差**
                       ( 4096,4096)  9.435 µs / 55.9%
                       ( 16384,1024) 8.624 µs / 61.3%  ← 5090 的峰在 S=4

5090 上 S=8 只剩 4 个 k 步 < `2×stages`,流水线损失主导;PPU 核少(72 vs 170),基线离"填满"更远,
**填充收益 + C-chain 消失(P 从 2.25 掉到 1)两个效应同向叠加**。⟹ **S 的曲线必须在 PPU 上自己扫,不许拿 5090 的结论代理**([[ppu-gemv-gs32-occupancy]])。

### 二、等价性(为什么这是 split-K 而不是别的)

    外部 reshape (32768,512)   256 个 tile × 4 个 k 步,一个 CTA 独吞
                               它的 32768 个输出**不是最终答案**,是 4096 个真输出的 8 份 partial
    内部 split-K S=8           32 个 n-tile × 8 片 = 256 个工作单元 × 4 个 k 步
                               ────────────────
                               分解完全相同,都还欠一次 8 路归约

**⟹ 用户测的 7.4 µs 是纯 GEMM 段。做法是 CUTLASS 2.x 的 `GemmSplitKParallel`:mainloop 不变,epilogue 改写 partial,外加归约。权重一个字节都不用动。**

### 三、要先确认的两件事(我算不准,需要用户或你补)

1. **那两次的 config 串**(TileN / TileK / warp 布局 / stages)。tile 数、tiles-per-CU、k 步数全靠它,现在我是按 TileN=128 假定的。
2. **是哪条 kernel。** standalone Marlin 被 `ClassicMainloopGeometryUnproved` 钉死在**唯一一个**几何(`tn!=128||tk!=128||warp_k!=32` 就拒),**不可能 sweep 出"不同的 winning config"** ⟹ 7.4 µs 大概率来自 **mixed-input dense 路**(`lowbit_dense_configs.inc` 那套)。**split-K collective 要加在产出那 7.4 µs 的那个 collective 上**,别加错地方。
...
* **(a) 逐位相同**:端到端输出与 `S=1` 路 `raw_bitdiff=0`。**硬判据,不是容差。**
* **(b) 端到端时间**:**必须报 GEMM + reduce 的总和**,与 `S=1` 的 17 µs 比。**只报 GEMM 段无效** —— 那正是用户已经知道的 7.4 µs。
* **(c) 曲线**:`S ∈ {1,2,4,8}` 每档都跑,每档报 `时间 / tile 数 / tiles-per-CU / k 步数 / %HBM / 权重每核每周期`。
* **(d) launch 开销**:单独量一次(空 kernel 或 reduce 单跑),写进报告 —— 它决定第二步值不值。
* **(e) acu**:`d000da88` 那条 `s.blksyn.defer`(S=1 时占全部采样 **31.71%**)的占比应当趋零。若不趋零,说明 P 没真的掉到 1,分解写错了。

### 七、负控(三条都要演示)

1. **`S=1` 必须复现 17 µs ±3%。** 对不上说明 harness 或 config 变了,先修那个再谈收益。
2. **把某一片的 slice 基址偏移植错一个 TileK** ⟹ (a) 必须红。
3. **把 reduce kernel 整个关掉** ⟹ (a) 必须红。若仍绿,说明测试没在看最终输出,**测试要先加强**。

### 八、范围限制

* **不改归约方式**(last-peer-wins / 原子加)—— 第二步。
* **不动权重格式、不动离线 artifact。**
* **S 先作为可调参数,不要默认打开 S=8。** 等 (c) 的曲线在 PPU 上扫完再定默认值。
* 5090 上 S=8 是负收益,**这不是反对做的理由,但报告里必须并列两台机器的曲线** —— 它是"配置最优点不可跨机沿用"的一个干净例证,值得留档。
