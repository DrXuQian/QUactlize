...
### 一、用户在 PPU 上的实测(冷缓存,两边各自 sweep 的最优,同 gs、同权重)
    ...
    (1,  4096, 4096)   19   µs     455      16.5%           7.2
                        ──────                              ────
                        2.57×                               2.6×
...
**归约 v1 故意选最笨的那个。** 预算是 `19 − 7.4 = 11.6 µs`,有这么大空间时先求「对且赢」不求最优;而且独立 kernel 能把 **launch 开销单独量出来**,那个数后面优化归约时还要用。
**优化归约(last-peer-wins / 原子加)是第二步,单独立项,不在本条。**

### 六、事前判据

* **(a) 逐位相同**:端到端输出与 `S=1` 路 `raw_bitdiff=0`。**硬判据,不是容差。**
* **(b) 端到端时间**:**必须报 GEMM + reduce 的总和**,与 `S=1` 的 19 µs 比。**只报 GEMM 段无效** —— 那正是用户已经知道的 7.4 µs。
...
1. **`S=1` 必须复现 19 µs ±3%。** 对不上说明 harness 或 config 变了,先修那个再谈收益。
