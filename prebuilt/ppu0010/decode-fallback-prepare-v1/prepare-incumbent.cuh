#pragma once
#include "/tmp/quactlize-tp2-fastpaths-20260919/quactlize/runtime/moe_chain.cuh"
#include "router-incumbent.cuh"

// Decode preparation with one router evaluation per token.
namespace quactlize::runtime::prepare_incumbent {
...
CUTLASS_DEVICE int token_router(qk_moe_plan_v1 const& plan,int token,int* ids) {
  ...
  return quactlize::llama::router_256_top8_warp_incumbent<HasBias,CoalescedIds>(router,io,ids+token*8);
}
