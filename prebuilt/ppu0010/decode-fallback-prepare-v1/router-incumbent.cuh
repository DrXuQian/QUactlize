#pragma once
#include "/tmp/quactlize-tp2-fastpaths-20260919/quactlize/integrations/llama/router.cuh"
...
CUTLASS_DEVICE float router_from_key_incumbent(unsigned key) {
...
CUTLASS_DEVICE int router_256_top8_warp_incumbent(qk_llama_router_v1 r,qk_llama_indexed_v1 io,int* shared_ids) {
  ...
  if constexpr (HasBias) {
    ...
    for (int slot=0;slot<8;++slot) {
      ...
      float weight=router_from_key_incumbent(maximum);
