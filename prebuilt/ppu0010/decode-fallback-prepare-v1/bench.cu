#define QK_PREPARE_PRODUCTION 1
...
static void ck(hggcError_t s) { if(s!=hggcSuccess) throw std::runtime_error(context+": "+hggcGetErrorString(s)); }
...
template<class T> struct Buffer {
  ...
  explicit Buffer(size_t n,bool weak=false):count(n),head(16+int(weak)) {
    ck(hggcMalloc(&base,(head+count+16)*sizeof(T)));ptr=base+head;reset();
  }
  ~Buffer() { if(base) hggcFree(base); }
  Buffer(Buffer const&)=delete;
  void reset() { ck(hggcMemset(base,0xa5,(head+count+16)*sizeof(T))); }
  void put(std::vector<T> const& v) {
    require(v.size()==count,"upload size");ck(hggcMemcpy(ptr,v.data(),count*sizeof(T),hggcMemcpyHostToDevice));
  }
  void put_on_stream(std::vector<T> const& v,hggcStream_t stream) {
    require(v.size()==count,"upload size");
    ck(hggcMemcpyAsync(ptr,v.data(),count*sizeof(T),hggcMemcpyHostToDevice,stream));
    ...
    ck(hggcStreamSynchronize(stream));
  }
  std::vector<T> get() const {
    std::vector<T> v(count);ck(hggcMemcpy(v.data(),ptr,count*sizeof(T),hggcMemcpyDeviceToHost));return v;
  }
  void guards() const {
    std::vector<unsigned char> a(head*sizeof(T)),b(16*sizeof(T));
    ck(hggcMemcpy(a.data(),base,a.size(),hggcMemcpyDeviceToHost));
    ck(hggcMemcpy(b.data(),ptr+count,b.size(),hggcMemcpyDeviceToHost));
...
#undef FAST
  ...
  void activation_proof(hggcStream_t stream) {
    ...
    g.put(gv);u.put(uv);ck(hggcDeviceSynchronize());
    ...
    ck(hggcGetLastError());ck(hggcStreamSynchronize(stream));
    auto out=d.get();
    ...
    moe_chain_swiglu_compute<<<dim3((n+255)/256,tokens*8),256,0,stream>>>(p);
    ck(hggcGetLastError());ck(hggcStreamSynchronize(stream));out=d.get();
  ...
  void correctness(bool candidate_only=false,int alias_offset=-1) {
    ...
    hggcStream_t stream;ck(hggcStreamCreateWithFlags(&stream,hggcStreamNonBlocking));
    hggcGraph_t graphs[2];hggcGraphExec_t exec[2];
    if(alias_offset>=0) plan.router.weights=logits.ptr+alias_offset;
    for(int arm=0;arm<2;++arm) {ck(hggcStreamBeginCapture(stream,hggcStreamCaptureModeGlobal));
      launch(arm,stream);ck(hggcStreamEndCapture(stream,&graphs[arm]));ck(hggcGraphInstantiate(&exec[arm],graphs[arm],nullptr,nullptr,0));}
    ...
    for(int repeat=0;repeat<4;++repeat) {
      ...
      if(candidate_only || alias_offset>=0) {
        ...
        ck(hggcDeviceSynchronize());
        if(plan.router.version) {router_control<<<tokens,32,0,stream>>>(plan);ck(hggcGetLastError());ck(hggcStreamSynchronize(stream));}
      ...
      for(int arm=int(candidate_only);arm<2;++arm) {
        ...
        ck(hggcDeviceSynchronize());
        ck(hggcGraphLaunch(exec[arm],stream));ck(hggcStreamSynchronize(stream));
      ...
      if(router_mode<0 && repeat==3) {
        for(int invalid:{-1,256,in[0]}) {
          ...
          for(int arm=int(candidate_only);arm<2;++arm) {ck(hggcGraphLaunch(exec[arm],stream));ck(hggcStreamSynchronize(stream));
            require(gate.header.get()[0].status && down.header.get()[0].status,"invalid IDs not rejected");gate.guards();up.guards();down.guards();}
        }
        ids.put_on_stream(in,stream);launch(int(candidate_only),stream);ck(hggcStreamSynchronize(stream));
      }
    }
    for(int a=0;a<2;++a) {ck(hggcGraphExecDestroy(exec[a]));ck(hggcGraphDestroy(graphs[a]));}
    ck(hggcStreamDestroy(stream));
  }
  void benchmark(int profile=-1) {
    hggcStream_t stream;ck(hggcStreamCreateWithFlags(&stream,hggcStreamNonBlocking));
    if(profile>=0) {
      launch(profile,stream);ck(hggcStreamSynchronize(stream));
      ck(hggcProfilerStart());
      for(int i=0;i<8;++i) launch(profile,stream);
      ck(hggcStreamSynchronize(stream));ck(hggcProfilerStop());
      ck(hggcStreamDestroy(stream));return;
    }
    hggcGraph_t graph[2];hggcGraphExec_t exec[2];hggcEvent_t begin,end;
    ck(hggcEventCreate(&begin));ck(hggcEventCreate(&end));
    for(int arm=0;arm<2;++arm) {ck(hggcStreamBeginCapture(stream,hggcStreamCaptureModeGlobal));
      for(int j=0;j<64;++j) launch(arm,stream);
      ck(hggcStreamEndCapture(stream,&graph[arm]));ck(hggcGraphInstantiate(&exec[arm],graph[arm],nullptr,nullptr,0));
      for(int j=0;j<5;++j) ck(hggcGraphLaunch(exec[arm],stream));}
    ck(hggcStreamSynchronize(stream));
    ...
    for(int round=0;round<4;++round) for(int sample=0;sample<15;++sample) for(int turn=0;turn<2;++turn) {
      ...
      ck(hggcEventRecord(begin,stream));ck(hggcGraphLaunch(exec[arm],stream));ck(hggcEventRecord(end,stream));
      ck(hggcEventSynchronize(end));float ms;ck(hggcEventElapsedTime(&ms,begin,end));samples[arm].push_back(ms*1000.f/64);
    }
    for(int arm=0;arm<2;++arm) {
      ...
      ck(hggcGraphExecDestroy(exec[arm]));ck(hggcGraphDestroy(graph[arm]));
    }
    ck(hggcEventDestroy(begin));ck(hggcEventDestroy(end));ck(hggcStreamDestroy(stream));
  }
};
...
static void router_edges() {
  ...
  for(int fixture=0;fixture<14;++fixture) for(int alias:{-1,0,8,248}) {
    ...
    for(int arm=0;arm<3;++arm) {
      ...
      ck(hggcGetLastError());ck(hggcDeviceSynchronize());
