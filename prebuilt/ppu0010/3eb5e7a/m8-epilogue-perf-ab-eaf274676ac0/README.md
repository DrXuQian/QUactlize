...
The manifest retains the exact inspector used at build time. Execution may use
a byte-different SDK inspector: the runner records its full identity and hash,
then requires that same executable to produce the exact ELF, resource, and ISA
evidence for both frozen payloads. No device binary was rebuilt for this
portable execution-inspector update.
...
```bash
ART_BRANCH=artifacts/ppu0010/3eb5e7a-m8-epilogue-perf-ab-eaf274676ac0
ART_REL=prebuilt/ppu0010/3eb5e7a/m8-epilogue-perf-ab-eaf274676ac0
