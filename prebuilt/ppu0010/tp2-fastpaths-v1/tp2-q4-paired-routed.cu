...
__global__ void point_12_1024_3072_arm_0(quactlize::fusion::DeviceCall c) {
    register_reuse_body<12,1,3,4,8,8,1,0,quactlize::fusion::SimtFinish>(c,1);
}
__global__ void point_12_1024_3072_arm_1(quactlize::fusion::DeviceCall c) {
    c.n=1024;c.k=3072;c.experts=256;c.mode=2;c.channels=1;c.topk=8;
    register_reuse_body<12,1,3,4,8,8,1,0,quactlize::fusion::SimtFinish>(c,1);
}
__global__ void point_12_1024_3072_arm_2(quactlize::fusion::DeviceCall c) {
    register_reuse_body<12,1,3,4,8,4,1,0,Paired16Finish>(c,1);
}
__global__ void point_12_1024_3072_arm_3(quactlize::fusion::DeviceCall c) {
    c.n=1024;c.k=3072;c.experts=256;c.mode=2;c.channels=1;c.topk=8;
    register_reuse_body<12,1,3,4,8,4,1,0,Paired16Finish>(c,1);
}
__global__ void point_12_1024_3072_arm_4(quactlize::fusion::DeviceCall c) {
    register_reuse_body<12,1,3,4,4,4,1,0,Paired16Finish>(c,1);
}
} // namespace
extern "C" int model_gemv_run(qkg_simt_call_v2 const* input,
    ...
    auto const& c=d.call;
    if(c.qtype!=12 || c.n!=512 || c.k!=3072 || c.experts!=256 ||
    ...
    case 0: {
        ...
        model_gemv::point_12_1024_3072_arm_0<<<c.rows*1*(1024/32),256,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
    ...
    case 1: {
        ...
        call.input_rows=fusion->input_rows;call.status=fusion->status;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_3072_arm_1<<<c.rows*1*(1024/32),256,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 2: {
        ...
        call.input_rows=fusion->input_rows;call.status=fusion->status;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_3072_arm_2<<<c.rows*1*(1024/16),256,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 3: {
        ...
        call.input_rows=fusion->input_rows;call.status=fusion->status;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_3072_arm_3<<<c.rows*1*(1024/16),256,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 4: {
        ...
        call.input_rows=fusion->input_rows;call.status=fusion->status;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_3072_arm_4<<<c.rows*1*(1024/16),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
