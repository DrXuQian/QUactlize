...
extern "C" int model_gemv_run(qkg_simt_call_v2 const* input,
    ...
    case 2: {
        ...
        call.input_rows=fusion->input_rows;call.status=fusion->status;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_2048_arm_2<<<c.rows*1*(1024/16),256,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 3: {
        ...
        call.input_rows=fusion->input_rows;call.status=fusion->status;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_2048_arm_3<<<c.rows*1*(1024/16),256,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 4: {
        ...
        call.input_rows=fusion->input_rows;call.status=fusion->status;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_2048_arm_4<<<c.rows*1*(1024/16),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
