...
__global__ void point_12_1024_3072_arm_0(qkg_call_v1 c) {
    register_reuse_body<12,1,3,4,4,4,1,0>(c,1);
}
__global__ void point_12_1024_3072_arm_1(qkg_call_v1 c) {
    register_reuse_body<12,1,3,4,4,4,1,3>(c,1);
}
__global__ void point_12_1024_3072_arm_2(qkg_call_v1 c) {
    c.n=1024;c.k=3072;c.experts=256;c.mode=2;c.channels=1;c.topk=8;
    register_reuse_body<12,1,3,4,4,4,1,3>(c,1);
}
__global__ void point_12_1024_3072_arm_3(qkg_call_v1 c) {
    register_reuse_body<12,1,3,8,4,4,1,3>(c,1);
}
} // namespace
extern "C" int model_gemv_run(qkg_simt_call_v2 const* input,
    ...
    auto const& d=*input;
    auto const& c=d.call;
    if(c.qtype!=12 || c.n!=1024 || c.k!=3072 || c.experts!=256 ||
    ...
    case 0: {
        ...
        model_gemv::point_12_1024_3072_arm_0<<<c.rows*1*(1024/16),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
    ...
    case 1: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_3072_arm_1<<<c.rows*1*(1024/16),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 2: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_3072_arm_2<<<c.rows*1*(1024/16),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 3: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_12_1024_3072_arm_3<<<c.rows*1*(1024/32),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
