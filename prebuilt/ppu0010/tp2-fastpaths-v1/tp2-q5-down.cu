...
__global__ void point_13_3072_512_arm_0(qkg_call_v1 c) {
    register_reuse_body<13,1,3,4,2,8,1,0>(c,1);
}
__global__ void point_13_3072_512_arm_1(qkg_call_v1 c) {
    register_reuse_body<13,1,3,4,2,8,1,3>(c,1);
}
__global__ void point_13_3072_512_arm_2(qkg_call_v1 c) {
    c.n=3072;c.k=512;c.experts=256;c.mode=2;c.channels=8;c.topk=8;
    register_reuse_body<13,1,3,4,2,8,1,3>(c,1);
}
__global__ void point_13_3072_512_arm_3(qkg_call_v1 c) {
...
extern "C" int model_gemv_run(qkg_simt_call_v2 const* input,
    ...
    auto const& d=*input;
    auto const& c=d.call;
    if(c.qtype!=13 || c.n!=3072 || c.k!=512 || c.experts!=256 ||
    ...
    case 0: {
        ...
        model_gemv::point_13_3072_512_arm_0<<<c.rows*1*(3072/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
    ...
    case 1: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_3072_512_arm_1<<<c.rows*1*(3072/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 2: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_3072_512_arm_2<<<c.rows*1*(3072/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 3: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_3072_512_arm_3<<<c.rows*1*(3072/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
