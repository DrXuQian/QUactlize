...
__global__ void point_8_2048_512_arm_0(qkg_call_v1 c) {
    q8_vector::kernel_body<1,0,1,8,4,4,true>(c,1);
}
__global__ void point_8_2048_512_arm_1(qkg_call_v1 c) {
    c.n=2048;c.k=512;c.experts=1;c.mode=0;c.channels=1;c.topk=1;
    q8_vector::kernel_body<1,0,1,8,4,4,true>(c,1);
}
__global__ void point_8_2048_512_arm_2(qkg_call_v1 c) {
    q8_vector::kernel_body<1,0,1,8,4,4,false>(c,1);
}
__global__ void point_8_2048_512_arm_3(qkg_call_v1 c) {
    q8_vector::kernel_body<1,0,1,8,2,4,true>(c,1);
}
__global__ void point_8_2048_512_arm_4(qkg_call_v1 c) {
    c.n=2048;c.k=512;c.experts=1;c.mode=0;c.channels=1;c.topk=1;
    q8_vector::kernel_body<1,0,1,8,2,4,true>(c,1);
}
__global__ void point_8_2048_512_arm_5(qkg_call_v1 c) {
    q8_vector::kernel_body<1,0,1,4,4,4,true>(c,1);
}
__global__ void point_8_2048_512_arm_6(qkg_call_v1 c) {
    q8_vector::kernel_body<1,0,1,4,2,4,true>(c,1);
}
} // namespace
extern "C" int model_gemv_run(qkg_simt_call_v2 const* input,
    ...
    auto const& d=*input;
    auto const& c=d.call;
    if(c.qtype!=8 || c.n!=2048 || c.k!=512 || c.experts!=1 ||
    ...
    case 0: {
        ...
        model_gemv::point_8_2048_512_arm_0<<<c.rows*1*(2048/32),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
    ...
    case 1: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_8_2048_512_arm_1<<<c.rows*1*(2048/32),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 2: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_8_2048_512_arm_2<<<c.rows*1*(2048/32),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 3: {
        qkg_simt_config_v1 cfg{1,sizeof(cfg),5,8,2,4,1};
        auto arrangement=q8_kpack2::arrangement();
        rc=simt::query_v2(d,cfg,&arrangement,sizes);if(rc) return rc;
        rc=simt::buffers_v2(d,sizes);if(rc) return rc;
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_8_2048_512_arm_3<<<c.rows*1*(2048/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 4: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_8_2048_512_arm_4<<<c.rows*1*(2048/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 5: {
        qkg_simt_config_v1 cfg{1,sizeof(cfg),5,4,4,4,1};
        auto arrangement=q8_kpack2::arrangement();
        rc=simt::query_v2(d,cfg,&arrangement,sizes);if(rc) return rc;
        rc=simt::buffers_v2(d,sizes);if(rc) return rc;
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_8_2048_512_arm_5<<<c.rows*1*(2048/16),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 6: {
        qkg_simt_config_v1 cfg{1,sizeof(cfg),5,4,2,4,1};
        auto arrangement=q8_kpack2::arrangement();
        rc=simt::query_v2(d,cfg,&arrangement,sizes);if(rc) return rc;
        rc=simt::buffers_v2(d,sizes);if(rc) return rc;
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_8_2048_512_arm_6<<<c.rows*1*(2048/16),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
