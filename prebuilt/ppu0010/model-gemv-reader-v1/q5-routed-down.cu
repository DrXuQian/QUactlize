...
__global__ void point_13_2048_512_arm_0(qkg_call_v1 c) {
    register_reuse_body<13,1,3,4,2,8,1,3>(c,1);
}
__global__ void point_13_2048_512_arm_1(qkg_call_v1 c) {
    c.n=2048;c.k=512;c.experts=256;c.mode=2;c.channels=8;c.topk=8;
    register_reuse_body<13,1,3,4,2,8,1,3>(c,1);
}
__global__ void point_13_2048_512_arm_2(qkg_call_v1 c) {
    register_reuse_body<13,1,3,4,2,4,1,3>(c,1);
}
__global__ void point_13_2048_512_arm_3(qkg_call_v1 c) {
    c.n=2048;c.k=512;c.experts=256;c.mode=2;c.channels=8;c.topk=8;
    register_reuse_body<13,1,3,4,2,4,1,3>(c,1);
}
__global__ void point_13_2048_512_arm_4(qkg_call_v1 c) {
    register_reuse_body<13,1,3,4,4,4,1,3>(c,1);
}
__global__ void point_13_2048_512_arm_5(qkg_call_v1 c) {
    register_reuse_body<13,1,3,8,2,4,1,3>(c,1);
}
__global__ void point_13_2048_512_arm_6(qkg_call_v1 c) {
    c.n=2048;c.k=512;c.experts=256;c.mode=2;c.channels=8;c.topk=8;
    register_reuse_body<13,1,3,8,2,4,1,3>(c,1);
}
} // namespace
extern "C" int model_gemv_run(qkg_simt_call_v2 const* input,
    ...
    auto const& d=*input;
    auto const& c=d.call;
    if(c.qtype!=13 || c.n!=2048 || c.k!=512 || c.experts!=256 ||
       c.mode!=2 || c.channels!=8 || c.topk!=8 ||
    ...
    switch(arm) {
    case 0: {
        qkg_simt_config_v1 cfg{1,sizeof(cfg),3,4,2,8,1};
        auto arrangement=ppu_arrangements::kquant_kpack_transpose_v1(13);
        ...
        model_gemv::point_13_2048_512_arm_0<<<c.rows*1*(2048/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
    ...
    case 1: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_2048_512_arm_1<<<c.rows*1*(2048/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 2: {
        qkg_simt_config_v1 cfg{1,sizeof(cfg),3,4,2,4,1};
        auto arrangement=ppu_arrangements::kquant_kpack_transpose_v1(13);
        rc=simt::query_v2(d,cfg,&arrangement,sizes);if(rc) return rc;
        rc=simt::buffers_v2(d,sizes);if(rc) return rc;
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_2048_512_arm_2<<<c.rows*1*(2048/16),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 3: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_2048_512_arm_3<<<c.rows*1*(2048/16),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 4: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_2048_512_arm_4<<<c.rows*1*(2048/16),128,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 5: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_2048_512_arm_5<<<c.rows*1*(2048/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
    case 6: {
        ...
        auto call=c;
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        model_gemv::point_13_2048_512_arm_6<<<c.rows*1*(2048/32),64,0,stream>>>(call);
        if(hggcGetLastError()!=hggcSuccess) return QKG_RUNTIME;
        return hggcGetLastError()==hggcSuccess ? QKG_OK : QKG_RUNTIME;
    }
