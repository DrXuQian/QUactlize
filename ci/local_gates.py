...
def lint_fq_splitk_partial_path():
    """Fixed Split-K publishes same-type FP32 partials without a shared handoff."""
    return _run_ci_script(
        "check_fq_splitk_partial_path.py",
...
def lint_fq_q4k_kpack4_delivery_ab():
    """K-pack4 delivery caps must preserve bytes/fragments and isolate shared-bank phase."""
    return _run_ci_script(
        "check_fq_q4k_kpack4_delivery_ab.py",
...
def lint_fq_q4k_kpack4_prefill_real_shapes():
    """K-pack4 prefill must sweep every registered M/family with full AP0."""
    return _run_ci_script(
        "check_fq_q4k_kpack4_prefill_real_shapes_runner.py",
