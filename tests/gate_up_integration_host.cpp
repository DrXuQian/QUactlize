...
static void graph_case(int tokens,int fault,int n=512,int k=2048) {
    auto * ctx=ggml_init({8*1024*1024,nullptr,true});
    assert(ctx);
    auto * a=ggml_new_tensor_2d(ctx,GGML_TYPE_F32,k,tokens);
    auto * gate=ggml_mul_mat(ctx,ggml_new_tensor_2d(ctx,fault==1?GGML_TYPE_Q4_K:GGML_TYPE_Q8_0,k,n),a);
    auto * up=ggml_mul_mat(ctx,ggml_new_tensor_2d(ctx,GGML_TYPE_Q8_0,k,n),fault==2?ggml_dup_tensor(ctx,a):a);
    ...
    assert(found==(tokens<=8 && fault==0 && n%256==0 && k%256==0));
    ggml_free(ctx);
}

int main() {
    using namespace quactlize::fusion;
    for(int tokens=1;tokens<=9;++tokens) for(int fault=0;fault<5;++fault) {
        graph_case(tokens,fault);
        graph_case(tokens,fault,1024,3072);
        graph_case(tokens,fault,256,512);
        graph_case(tokens,fault,128,512);
        if(!fault) graph_case(tokens,fault,512,32);
    }
    for(int q:{8,12}) for(int tokens=1;tokens<=8;++tokens) {
        ...
        assert(paired_shape(q,1024,3072,e,tokens,compute));
        assert(paired_shape(q,256,512,e,tokens,compute));
        assert(!paired_shape(q,128,3072,e,tokens,compute));
        assert(!paired_shape(q,512,32,e,tokens,compute));
        assert(!paired_shape(q,INT32_MAX,3072,e,tokens,compute));
    ...
    std::puts("GATE_UP_INTEGRATION_HOST PASS graph=189 selection=16 negative_arms=covered");
}
