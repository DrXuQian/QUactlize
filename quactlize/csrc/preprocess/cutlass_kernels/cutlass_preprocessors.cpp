...
int get_bits_in_quant_type(QuantType quant_type)
{
    switch (quant_type) {
        ...
        case QuantType::PACKED_INT4_WEIGHT_ONLY:
            return 4;
        case QuantType::PACKED_INT2_WEIGHT_ONLY:
            return 2;
        default:
...
LayoutDetails getLayoutDetailsForArch(QuantType quant_type)
{
    ...
    else if (quant_type == QuantType::PACKED_INT2_WEIGHT_ONLY) {
        // box-verify: needs cutlass LayoutDetailsB<uint2b_t>. If unspecialized, the interleaved path won't
        // compile -> use the NON-interleaved (ColumnMajor) driver path first (n,k not %256 -> il=false).
        details = getLayoutDetailsForArchAndQuantType<cutlassArch, cutlass::uint2b_t>();
    }
    else {
...
LayoutDetails getLayoutDetailsForTransform(QuantType quant_type, const int arch)
{
    if (arch >= 70 && arch < 75) {
                                   ...
                                   bool                       is_int8_mma)
{
    ...
    FT_CHECK(quant_type == QuantType::PACKED_INT4_WEIGHT_ONLY || quant_type == QuantType::INT8_WEIGHT_ONLY || quant_type == QuantType::PACKED_INT2_WEIGHT_ONLY);
                            ...
                            const std::vector<size_t>& shape)
{
    ...
    static_assert(quant_type == QuantType::INT8_WEIGHT_ONLY || quant_type == QuantType::PACKED_INT4_WEIGHT_ONLY
                  || quant_type == QuantType::PACKED_INT2_WEIGHT_ONLY, "");
    static constexpr int ELTS_PER_BYTE = quant_type == QuantType::INT8_WEIGHT_ONLY ? 1
                                       : (quant_type == QuantType::PACKED_INT4_WEIGHT_ONLY ? 2 : 4);
        ...
        for (size_t row_tile_start = 0; row_tile_start < num_rows; row_tile_start += M_TILE_L1) {
            for (size_t col_tile_start_byte = 0; col_tile_start_byte < col_bytes; col_tile_start_byte += N_TILE_L1) {
                ...
                if (quant_type == QuantType::INT8_WEIGHT_ONLY) {
                    ...
                    // 2-bit crumb transpose: mirror of the int4 nibble path with ELTS_PER_BYTE=4 (>>2*off, mask 0x3).
                    // box-verify (subbyte transpose of 4 crumbs/byte).
                    for (int ii = 0; ii < M_TILE_L1; ++ii) {
                        for (int jj = ii + 1; jj < M_TILE_L1; ++jj) {
                            const int ii_byte       = ii / ELTS_PER_BYTE;
                            const int ii_bit_offset = ii % ELTS_PER_BYTE;
                       ...
                       QuantType                  quant_type)
{
    ...
    if (quant_type == QuantType::INT8_WEIGHT_ONLY) {
        ...
        subbyte_transpose_impl<QuantType::PACKED_INT2_WEIGHT_ONLY>(
            transposed_quantized_tensor, quantized_tensor, shape);
    }
    else {
                                       ...
                                       const int                  arch)
{
    ...
    if (details.uses_imma_ldsm) {
        permute_B_rows_for_mixed_gemm(dst_buf.data(), src_buf.data(), shape, quant_type, arch, is_int8_mma);
...
#if defined(USE_AIU)
    // aiu fpa intb column interleaved layout preprocess, currently only used in a16w4.
    if (aiu_interleaved && quant_type == QuantType::PACKED_INT4_WEIGHT_ONLY) {
        // details.rows_per_column_tile = 128 * 8 / get_bits_in_quant_type(quant_type);
        details.rows_per_column_tile = 256;
        const size_t num_rows    = shape.size() == 2 ? shape[0] : shape[1];     // k
        const size_t num_cols    = shape.size() == 2 ? shape[1] : shape[2];     // n
                        ...
                        const int                  arch)
{
    ...
    preprocess_weights_for_mixed_gemm(processed_quantized_weight, unprocessed_quantized_weight, shape, quant_type, false, false, arch);
}

template void
symmetric_quantize<float, float>(int8_t*, int8_t*, float*, const float*, const std::vector<size_t>&, QuantType, const int);

template void
symmetric_quantize<half, float>(int8_t*, int8_t*, half*, const float*, const std::vector<size_t>&, QuantType, const int);

template void
symmetric_quantize<half, half>(int8_t*, int8_t*, half*, const half*, const std::vector<size_t>&, QuantType, const int);

#ifdef ENABLE_BF16
template void symmetric_quantize<__nv_bfloat16, __nv_bfloat16>(
    int8_t*, int8_t*, __nv_bfloat16*, const __nv_bfloat16*, const std::vector<size_t>&, QuantType, const int);

template void symmetric_quantize<__nv_bfloat16, half>(
    int8_t*, int8_t*, __nv_bfloat16*, const half*, const std::vector<size_t>&, QuantType, const int);

template void symmetric_quantize<half, __nv_bfloat16>(
    int8_t*, int8_t*, half*, const __nv_bfloat16*, const std::vector<size_t>&, QuantType, const int);

template void symmetric_quantize<__nv_bfloat16, float>(
    int8_t*, int8_t*, __nv_bfloat16*, const float*, const std::vector<size_t>&, QuantType, const int);
#endif
...
void symmetric_quantize(int8_t*                    processed_quantized_weight,
                        ...
                        const int                  arch)
{
    symmetric_quantize(processed_quantized_weight, nullptr, scale_ptr, input_weight_ptr, shape, quant_type, arch);
}

template void symmetric_quantize<float, float>(int8_t*, float*, const float*, const std::vector<size_t>&, QuantType, const int);

template void symmetric_quantize<half, float>(int8_t*, half*, const float*, const std::vector<size_t>&, QuantType, const int);

template void symmetric_quantize<half, half>(int8_t*, half*, const half*, const std::vector<size_t>&, QuantType, const int);

#ifdef ENABLE_BF16
template void symmetric_quantize<__nv_bfloat16, __nv_bfloat16>(
    int8_t*, __nv_bfloat16*, const __nv_bfloat16*, const std::vector<size_t>&, QuantType, const int);

template void
symmetric_quantize<__nv_bfloat16, half>(int8_t*, __nv_bfloat16*, const half*, const std::vector<size_t>&, QuantType, const int);

template void
symmetric_quantize<half, __nv_bfloat16>(int8_t*, half*, const __nv_bfloat16*, const std::vector<size_t>&, QuantType, const int);

template void
symmetric_quantize<__nv_bfloat16, float>(int8_t*, __nv_bfloat16*, const float*, const std::vector<size_t>&, QuantType, const int);
#endif
} // namespace cutlass_kernels
} // namespace kernels
}  // namespace acext
