...
//
// Isolated host/type builder for the grouped Marlin mechanism gate.  It is
...
struct Operation {
  ...
  static_assert(
      ...
      "grouped Marlin epilogue M layout must match MMA atom and M warps");
  using Kernel = cutlass::gemm::kernel::GroupMarlinMixedInputKernel<
 ...
 public:
  ...
  static Arguments make_arguments(
    ...
    static_assert(ArtifactTileK > 0,
                  "the isolated Marlin gate requires an explicit artifact TileK");
  ...
  static Plan inspect(Arguments const& args, void* workspace) {
    ...
    out.grid_blocks = params.scheduler.grid_blocks_;
    out.active_blocks = params.scheduler.active_blocks_;
    out.iters_per_block = params.scheduler.iters_per_block_;
    for (uint64_t q = 0; q < params.scheduler.output_tiles_; ++q) {
      uint64_t const first =
          (q * params.scheduler.k_tiles_per_output_) /
          params.scheduler.iters_per_block_;
      uint64_t const last =
          ((q + 1) * params.scheduler.k_tiles_per_output_ - 1) /
          params.scheduler.iters_per_block_;
      out.handoffs += last - first;
    }
