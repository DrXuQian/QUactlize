...
inline int validate(qk_call_v1 const& c, qk_recipe_v1 const& r, int& max_rows) {
  ...
  if constexpr (grouped) {
    if (!c.rows_host || r.split != 1) return QK_INVALID;
    int64_t total = 0;
    max_rows = 0;
...
template<bool Persistent> struct GroupedHandle final : Handle {
  ...
  int prepare(qk_call_v1 const& c, qk_recipe_v1 const& r, int occupancy, int maximum) {
    call = c;
    max_rows = maximum;
    auto stream = static_cast<hggcStream_t>(c.stream);
    ...
    int64_t offset = 0;
    bool uniform = true;
    int first_tiles = (c.rows_host[0] + tm - 1) / tm;
    ...
    if (!copy(ds,shapes.data(),sizeof(Shape)*c.experts) ||
    ...
    problem.num_groups=c.experts; problem.problem_shapes=ds; problem.host_problem_shapes=shapes.data();
    ...
    if constexpr (Persistent) {
      ...
      args.group_M=c.rows_device; args.mtiles_uniform=uniform?first_tiles:0; args.splitk=1;
    }
    if (G::can_implement(args) != cutlass::Status::kSuccess) return QK_UNSUPPORTED;
    ...
    if constexpr (!Persistent)
      if (!uniform && !copy(base,prefix.data(),prefix.size()*sizeof(int))) return QK_RUNTIME_ERROR;
    return QK_OK;
  }
  int run(hggcStream_t stream) override {
    if constexpr (Persistent) {
...
inline int query(qk_call_v1 const& c,qk_recipe_v1 const& r,qk_resources_v1& out) {
  out={1,sizeof(out),0,0,0,0,0};
  int maximum=0, status=validate(c,r,maximum);
