...
template<int Input,int Reader,int Variant,int Warps,int P,int Columns,int N,int K>
__global__ void kernel_bf16(qkg_call_v1 c) {
    constexpr int TileN=Reader==QKG_Q4_META ? 8 : Columns*P;
    static_assert(Columns*P<=32);
    int row=blockIdx.x/(N/TileN),tile=blockIdx.x%(N/TileN);
    Row r=locate(c,row);
    ...
    auto low=c.low+uint64_t(r.expert)*N*K/2;
    auto units=c.units+uint64_t(r.expert)*N*K/16;
    if constexpr(Reader==QKG_Q4_META) row_meta<Input,Variant,1,1,Warps,N,K,1>(tile,a,low,units,out);
    else if constexpr(Reader==QKG_Q4_MEDIUM)
        row_medium<Input,Variant&1,(Variant>>1)&1,1,P==2,P==2,Columns,Warps,P,N,K,1>(tile,a,low,units,out);
    else row_reuse<Input,Variant,Columns,Warps,P,N,K,1>(tile,a,low,units,out);
}

template<int R,int V,int W,int P,int Columns,int N,int K,int Compute=0>
int launch(qkg_call_v1 const& c) {
    ...
    if constexpr(Compute==0)
        ...
        kernel_bf16<1,R,V,W,P,Columns,N,K><<<c.rows*(N/TileN),W*32,0,stream>>>(c);
    else if (c.input_type==QKG_SIMT_BF16)
        kernel_bf16<2,R,V,W,P,Columns,N,K><<<c.rows*(N/TileN),W*32,0,stream>>>(c);
