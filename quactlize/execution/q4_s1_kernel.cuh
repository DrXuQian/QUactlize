...
template<int Input,int Reader,int Variant,int Warps,int P,int N,int K>
__global__ void indexed_kernel_bf16(qkg_call_v1 c) {
    constexpr int TileN=Reader==QKG_Q4_META ? 8 : 4*P;
    int const row=blockIdx.x/(N/TileN),tile=blockIdx.x%(N/TileN);
    Row const r=locate(c,row);
    float* out=c.output+r.output;
    ...
    using A=ComputeActivation<Input,1>;
    A const a{static_cast<typename A::Scalar const*>(c.a)+r.a};
    uint8_t const* low=c.low+uint64_t(r.expert)*N*K/2;
    uint8_t const* units=c.units+uint64_t(r.expert)*N*K/16;
    if constexpr(Reader==QKG_Q4_META)
        row_meta<Input,Variant,1,1,Warps,N,K,1>(tile,a,low,units,out);
    else if constexpr(Reader==QKG_Q4_MEDIUM)
        row_medium<Input,Variant&1,(Variant>>1)&1,1,P==2,P==2,4,Warps,P,N,K,1>(tile,a,low,units,out);
    else {
        static_assert(Reader==QKG_Q4_REUSE && (Variant==6 || Variant==7));
        row_reuse<Input,Variant,4,Warps,P,N,K,1>(tile,a,low,units,out);
    }
}

template<int Reader,int Variant,int Warps,int P,int N,int K,int Compute=0>
int launch(qkg_call_v1 const& c) {
    ...
    if constexpr(Compute==0) {
        ...
        if(c.input_type==QKG_F16)
            indexed_kernel_bf16<0,Reader,Variant,Warps,P,N,K><<<grid,Warps*32,0,stream>>>(c);
        else if(c.input_type==QKG_F32)
            indexed_kernel_bf16<1,Reader,Variant,Warps,P,N,K><<<grid,Warps*32,0,stream>>>(c);
        else if(c.input_type==QKG_SIMT_BF16)
            indexed_kernel_bf16<2,Reader,Variant,Warps,P,N,K><<<grid,Warps*32,0,stream>>>(c);
